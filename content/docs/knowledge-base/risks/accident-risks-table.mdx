---
wikiId: E659
title: Accident Risks Table
description: Interactive comparison table of AI accident and misalignment risks with overlap analysis.
subcategory: accident
contentFormat: table
lastEdited: 2026-02-10
update_frequency: 30
summary: An interactive sortable table comparing 16 AI accident risks across dimensions including abstraction level, evidence, timeline, severity, and detectability. Shows risk relationships and supports grouped/unified views.
clusters:
  - ai-safety
---
import AccidentRisksTableView from '@components/tables/views/AccidentRisksTableView';

<AccidentRisksTableView />
