...
Large AI training clusters require hundreds of megawatts of power. The <EntityLink id="projecting-compute-spending">projected incremental power demand</EntityLink> of 15-20 GW in 2026 alone[^rc-8d9b] creates dependencies on specific power grid infrastructure. A targeted attack on power substations serving major data center clusters — a threat that has materialized in physical attacks on US power infrastructure in recent years — could disable AI training capacity without directly breaching any network security.
