...
**"Reliability API"**: Provide a public API that other tools can query. A browser could check the reliability score of every author on a page. An AI chatbot could weight sources by their reliability scores when doing retrieval. <EntityLink id="community-notes-for-everything">Community Notes</EntityLink> could use reliability data to prioritize which content to annotate.
