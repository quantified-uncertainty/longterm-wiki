...
<DataInfoBox entityId="maim" />

## Overview

Mutually Assured AI Malfunction (MAIM) is a deterrence framework for managing great-power competition over advanced AI. Introduced by <EntityLink id="dan-hendrycks" /> (director of <EntityLink id="cais" />), Eric Schmidt (former Google CEO), and Alexandr Wang (Scale AI CEO) in their March 2025 paper [*Superintelligence Strategy*](https://arxiv.org/abs/2503.05628), MAIM proposes that any state's aggressive bid for unilateral AI dominance will be met with preventive sabotage by rivals.
...
The deterrence pillar is most novel and controversial. The nonproliferation and competitiveness pillars build on existing policy proposals around <EntityLink id="compute-governance" />, <EntityLink id="export-controls" />, and <EntityLink id="hardware-enabled-governance" />.
...
<EntityLink id="miri" /> published a detailed [analysis applying formal deterrence theory](https://intelligence.org/2025/04/11/refining-maim-identifying-changes-required-to-meet-conditions-for-deterrence/) to MAIM, finding:
...
- **<EntityLink id="compute-governance" />**: The nonproliferation pillar relies on controlling access to AI-relevant compute
- **<EntityLink id="export-controls" />**: Current US chip restrictions to China are a precursor to MAIM-style nonproliferation
- **<EntityLink id="coordination-mechanisms" />**: MAIM's escalation ladders and verification proposals complement international coordination efforts
- **<EntityLink id="racing-dynamics" />**: MAIM attempts to address the same competitive pressures that drive <EntityLink id="racing-dynamics" />
- **<EntityLink id="pause" />**: Some critics argue that advocating for compute pauses or development moratoria would be more effective than deterrence
...
- The framework could eventually evolve into something resembling <EntityLink id="international-regimes" /> for AI governance
