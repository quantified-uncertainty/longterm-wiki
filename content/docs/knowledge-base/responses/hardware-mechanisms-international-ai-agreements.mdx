...
**TRL assessment:** TRL 3–4. The underlying cryptographic primitives (secure enclaves, remote attestation protocols) exist in commercial silicon, but no production AI accelerator currently ships with verified, tamper-resistant location reporting designed for treaty purposes. The bipartisan Chip Security Act (May 2024) would have mandated such features; it has not passed as of early 2026.

**Active developers:** <EntityLink id="E202">MIRI</EntityLink> SPAR projects led by Aaron Scher; arXiv work by O'Gara, Kulp, et al.; US government interest through the Trump AI Action Plan (July 2025).

**What remains:** Integrating location attestation into chip fabrication pipelines, hardening against spoofing attacks, resolving privacy concerns about continuous location reporting, and establishing the multilateral key infrastructure needed for third-party verification.

**Estimated timeline:** Pilot-ready within 3–5 years for export control verification if a major chip manufacturer (e.g., NVIDIA, TSMC) commits to integration; broader treaty-grade deployment likely 5–10 years out.[^2][^3]
...
**TRL assessment:** TRL 5–6 for commercial confidential computing (NVIDIA H100 already supports some TEE functionality). TRL 2–3 for treaty-grade AI monitoring applications, which require additional protocol design, mutual agreement on approved code, and independent evaluation of the TEE's security assumptions.

**Active developers:** NVIDIA (commercial); Oxford Martin AIGI researchers; O'Gara, Kulp, et al. (protocol design); the "FlexHEG" (Flexible Hardware-Enabled Guarantee) approach developed by Petrie et al. targets faster on-chip location and compute attestation without full chip redesign.[^3]

**What remains:** Standardizing what an AI monitoring TEE must certify (compute quantity, cluster configuration, model behavior properties); conducting adversarial red-teaming of current TEE implementations; establishing mutual evaluation protocols between nations; and resolving IP protection concerns that arise when model internals are proximate to a verified computation.
...
**Active developers:** O'Gara, Kulp, et al. (arXiv 2025); <EntityLink id="E202">MIRI</EntityLink> SPAR projects; Joël N. Christoph (European University Institute, Harvard Kennedy School) on market-based compute permit schemes that rely on hardware-audited usage.
