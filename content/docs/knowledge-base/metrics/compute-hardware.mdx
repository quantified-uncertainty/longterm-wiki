...
update_frequency: 21
llmSummary: "A comprehensive metrics reference tracking GPU production (NVIDIA 80-90% market share, 4-7M units/year by 2025), training compute growth (4-5x annually, 30+ models at GPT-4 scale by mid-2025), algorithmic efficiency gains (2x every 8 months), and AI power consumption (40 TWh in 2024, projected 945 TWh by 2030), with detailed coverage of TSMC fab capacity, China's constrained domestic production, and cost trajectories. The page synthesizes publicly available data from Epoch AI, IEA, and industry sources into a structured reference with explicit data quality ratings and uncertainty acknowledgments."
ratings:
  focus: 8.5
  novelty: 4.5
  rigor: 6.5
...
**Key Findings:**
- **Training compute growth**: Reportedly a 4–5× annual increase since 2010, with reportedly 30+ models reaching GPT-4 scale (10²⁵ FLOP) by mid-2025, according to some sources tracking frontier model development
- **Market concentration**: <EntityLink id="E693" name="nvidia">NVIDIA</EntityLink> reportedly maintains 80–90% share of the AI accelerator market, driven by CUDA ecosystem integration and sustained performance leadership
- **Efficiency gains**: According to some analyses, algorithmic improvements reduce effective compute requirements by roughly 50% every 8 months (95% CI: 5–14 months), a pace exceeding Moore's Law
- **Energy trajectory**: AI workloads reportedly consumed approximately 40 TWh in 2024 (around 15% of global data center electricity), with some projections reaching 945 TWh by 2030 assuming ~15% annual growth
- **Supply constraints**: Advanced packaging (CoWoS) and HBM memory are widely reported to present greater production bottlenecks than raw wafer capacity
- **China's production**: Domestic chip output is reportedly constrained by 20–50% yields on 7nm processes and limited HBM access, despite plans for 600k+ chips in 2025, according to some industry sources
- **Cost trends**: Training costs for frontier models reportedly triple annually, while equivalent-performance training costs reportedly decline roughly 10× per year through efficiency improvements, according to some analyses

> **Note on citations**: The specific figures above are widely cited in AI policy and research literature but originate from a range of primary sources (including <EntityLink id="E125" name="epoch-ai">Epoch AI</EntityLink>, IEA, and industry analyst reports). Detailed citations for each claim appear in the corresponding sections below.
...
Hardware availability directly constrains AI capabilities development. Training compute determines which models can be built, efficiency improvements affect economic viability of deployment, and supply chain dynamics influence geopolitical competition. These metrics inform policy decisions including <EntityLink id="export-controls">export controls</EntityLink>, <EntityLink id="compute-governance">compute governance</EntityLink> proposals, and infrastructure planning.
...
flowchart TD
    subgraph DESIGN["Chip Design"]
        ...
        GOOGLE[<EntityLink id="E98" name="deepmind">Google DeepMind</EntityLink> TPU<br/>Internal use]
    end
...
- <EntityLink id="meta-ai">Meta</EntityLink>: 13% of global AI compute share
...
- **<EntityLink id="eu-ai-act">EU AI Act</EntityLink>**: 10²⁵ FLOP reporting requirement
- **<EntityLink id="us-executive-order">US Executive Order 14110</EntityLink>**: 10²⁶ FLOP reporting requirement
...
The cost of compute has declined dramatically over the past decade, with AI-specific hardware advancing faster than general semiconductor trends.

**Key Metrics** (figures are estimates and vary by source):
- **Overall decline (2019–2025)**: FP32 FLOP cost reportedly decreased roughly 74%, though exact figures depend on methodology
- **AI training cost decline**: According to some estimates, training costs have fallen approximately 10x per year on comparable tasks — roughly 50x faster than classical Moore's Law predictions
- **GPU price-performance**: Reportedly doubling roughly every 16 months on frontier chips, according to some industry analyses

**Historical Training Cost Examples** (approximate, methodology-dependent):
- ResNet-50 image recognition: reportedly ≈\$1,000 (2017) → ≈\$10 (2019)
- ImageNet 93% accuracy: training cost reportedly halving roughly every 9 months (2012–2022), according to some estimates
- GPT-4-scale training: reportedly ≈\$100M (2023); later runs on improved hardware reportedly reduced equivalent costs substantially, though specific figures vary widely by source

**GPU Generation Improvements** (approximate):
- A100 → H100: reportedly ~2x price-performance improvement
- Google TPU v5p vs. v4: reportedly ~30% throughput improvement and ~25% lower energy, according to some sources

**Data Quality**: Medium — historical data is better established; projections and specific figures carry significant uncertainty.

> ⚠️ No primary sources were available in the source cache to verify specific numbers in this section. All quantitative claims should be treated as approximate estimates pending citation.
...
The split between training and inference workloads at major AI labs is difficult to measure precisely, as most labs do not publish utilization data. Figures cited in industry analyses—such as an approximately 80% training / 20% inference split currently, reversing to roughly 30% training / 70% inference by 2030—are reportedly based on cost modeling rather than disclosed metrics and should be treated as indicative estimates.

**Lab-Specific Data**:

Similarly, figures sometimes cited for OpenAI's 2024 compute costs (e.g., ≈\$3B training, ≈\$1.8B inference, ≈\$1B research) and claims that inference becomes 15–118× more expensive than training over a model's lifetime are unverified and have not been confirmed by OpenAI publicly.

A Google internal study (2019–2021) reportedly found that inference accounted for roughly 60% of total ML compute in three-week snapshots, though this figure comes from secondary accounts rather than a public disclosure.

**Utilization Challenges** (broadly acknowledged in industry):
- Packaging bottlenecks (e.g., CoWoS advanced packaging)
- HBM memory supply constraints
- Infrastructure development lag relative to accelerator deployment

**Data Quality**: Low. Most labs do not publish utilization rates; available estimates derive from cost reports and modeling exercises of uncertain provenance.

## 8. Inference vs. Training Compute Ratio

**Current State (circa 2024)**:

The precise split between training and inference compute varies by organization and reporting period. Some industry observers have suggested roughly 80% training and 20% inference during recent large-model training surges, though no major lab has published audited figures. Claims that OpenAI generates approximately 100 billion tokens per day (≈36 trillion per year) and that training tokens cost roughly 3× more per token than inference tokens are reportedly circulating in analyst discussions but remain unverified by primary sources.

**Reported Historical Evolution**:

| Period | Inference Share | Training Share | Source Note |
|--------|----------------|----------------|-------------|
| 2019–2021 (Google) | ≈60% | ≈40% | Reportedly based on 3-week internal snapshots; unverified externally |
| 2024 (Industry est.) | ≈20% | ≈80% | Analyst estimates during training surge; no audited disclosure |
| 2030 (Projection) | ≈70% | ≈30% | Speculative post-surge equilibrium; no established source |

**Theoretical Compute Allocation**:

Epoch AI has explored optimal compute allocation between training and inference under parameterized models. The qualitative intuition is that unless training and inference returns differ by an order of magnitude, rational actors will allocate meaningfully to both. Specific parameter values (e.g., α thresholds of 0.1 or 10) remain tied to particular modeling assumptions and should not be treated as empirical findings.

**Inference Growth Drivers**:
- Deployment at scale creates ongoing inference demand versus one-time training cost
- Inference demand grows with user base and query volume

**Data Quality**: Low–Medium. Specific figures lack primary-source verification and should be treated as rough estimates only.
...
- <EntityLink id="E269" name="sam-altman">Sam Altman</EntityLink> has reportedly stated that GPT-4 cost "more than \$100M" to train, though no official figure has been published by OpenAI.[^1]
- Independent estimates from Epoch AI reportedly placed hardware and energy costs alone at approximately \$40M, with full cost estimates ranging from \$78M to \$192M depending on methodology — though these figures have not been formally verified.[^2]

**GPT-4-Equivalent Training Costs (Optimized)**:
- According to some analyst estimates, by Q3 2023 a GPT-4-equivalent model could reportedly be trained for approximately \$20M due to efficiency improvements.[^3]
- The Chinese AI startup 01.ai reportedly claimed to have trained a comparable model for approximately \$3M using around 2,000 GPUs and various optimizations, though this claim has not been independently audited.[^4]

### Cost Trend Analysis

**Training Cost Growth (Frontier Models)**:
- Historical analysis suggests frontier training costs have been growing rapidly — <EntityLink id="E91" name="dario-amodei">Dario Amodei</EntityLink> stated in August 2024 that some models were already costing around \$1B to train, and projected \$10B training runs by 2025, though these figures should be treated as forward-looking estimates.[^5]
- Algorithmic and hardware efficiency gains are reportedly compounding, with some researchers estimating roughly a 2x efficiency improvement every 9 months from algorithms alone.[^6]

### When Will GPT-4 Training Cost Under \$1M?

Projections in this domain are highly uncertain and depend on assumptions about both algorithmic progress and hardware price trends. Rather than presenting specific year-by-year figures as reliable forecasts, the general expectation among analysts is that:

- Achieving GPT-4-level *performance* will become substantially cheaper over time as algorithmic improvements compound.
- Frontier model costs will simultaneously rise as labs pursue greater capabilities.
- The trend is divergent: equivalent performance gets cheaper while cutting-edge capability gets more expensive.

Specific projection timelines (e.g., sub-\$1M GPT-4-equivalent training by 2025–2027) have been circulated but cannot be reliably sourced and are omitted here pending better documentation.

**Data Quality**: Low–Medium. Based on partial, unaudited cost disclosures and analyst extrapolations.
...
During the chip shortage peak, lead times for lithography equipment reportedly reached extreme levels, according to industry analyses and supply chain reports:
- **ArF immersion equipment**: reportedly ~24 months
- **EUV equipment**: reportedly ~18 months
- **I-line equipment**: reportedly ~18 months
- **Industry average** (all equipment): reportedly ~14 months, up from an estimated 3–6 months pre-shortage

These figures circulated widely in trade press and analyst reports but have not been formally confirmed in ASML's official filings.

**Current State (2024–2025)**:
- Lead times have moderated from 2022 peaks but remain, according to some industry observers, "incredibly long"
- Foundries must plan capacity expansions well in advance
- Exact current lead times are not publicly disclosed by ASML
...
| **EUV High-NA (0.55 NA)** | — | reportedly ≈20 systems/year |

ASML has publicly discussed capacity expansion ambitions, though specific annual targets above are drawn from analyst summaries and may reflect aspirational rather than committed figures.

**2024 Shipments** (Actual, per ASML's published annual results):
- Total lithography: reportedly 418 systems
- EUV: reportedly 44 systems
- DUV: reportedly 374 systems
- Metrology/inspection: reportedly 165 systems

**High-NA EUV Systems**:
- **Cost**: reportedly \$400M+ per system (vs. approximately \$200M for low-NA EUV), according to various industry sources
- **First commercial deployment**: Intel's TWINSCAN EXE:5200B
- **Status**: Transition from low-NA to high-NA beginning 2024–2025

### Market Concentration

**ASML Market Position**:
- ASML holds an approximately 94% share of the lithography equipment market as of 2024, according to some analyst estimates; the remaining share is split between Canon and Nikon
- ASML is the **exclusive supplier** of EUV lithography systems globally

### Geopolitical Constraints

**China Export Restrictions**:
- ASML has stated publicly that it expects China customer demand to decline significantly in 2026 relative to 2024–2025 levels
- However, ASML has indicated total 2026 net sales are not expected to fall below 2025 levels, as non-China growth is expected to compensate

**China's EUV Development**:
- Reports have emerged of prototype domestic EUV lithography development in China, with a stated target of AI chip output by 2028
- Independent verification is limited; observers widely consider domestic Chinese EUV to be far from production capability

**Lead Time Implications**:
- Long lead times structurally favor incumbents with existing tool allocations
- New entrants, especially those subject to geopolitical restrictions, face multi-year delays in accessing advanced lithography
- Supply constraints on advanced packaging (e.g., CoWoS) are now considered by some analysts to be more immediately binding than lithography bottlenecks

**Data Quality**: Medium. Core figures are drawn from analyst reports, trade press, and ASML public statements; several specific numbers lack primary-source confirmation and are hedged accordingly.
...
[^1]: Sam Altman's "more than \$100M" figure is widely cited in press coverage but originates from informal statements; no official OpenAI disclosure has been published. Treat as reportedly stated.
[^2]: The \$40M hardware/energy and \$78–\$192M full-cost range are attributed to Epoch AI analyses in secondary reporting. Primary source documentation is not publicly confirmed at time of writing.
[^3]: The ≈\$20M Q3 2023 estimate for a GPT-4-equivalent training run appears in analyst commentary and has not been formally published in a peer-reviewed source.
[^4]: The ≈\$3M / 2,000 GPU claim is attributed to 01.ai's public statements and has not been independently verified.
[^5]: Dario Amodei's \$1B/\$10B projections were reportedly made in August 2024; the precise sourcing (interview, essay, or presentation) should be confirmed before treating these as authoritative.
[^6]: Algorithmic efficiency doubling roughly every 9 months is a figure cited in AI scaling literature, but specific estimates vary across studies.
