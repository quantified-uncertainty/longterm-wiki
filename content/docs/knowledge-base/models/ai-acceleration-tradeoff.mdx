...
**Central framing**: "Speeding up TAI arrival by 1% is estimated to increase the probability of existential catastrophe by 0.3-1.5 percentage points, while adjusting the conditional value of the long-term future by -0.2% to +0.5%."
...
| Delay cost | $C(\Delta T)$ | Annual cost of delayed TAI benefits |
...
Since capability at deployment is roughly fixed (TAI is TAI regardless of when it arrives), the key determinant is how much time safety and governance have had to prepare. More time generally means lower risk, but with diminishing returns as readiness approaches sufficiency.
...
These estimates imply that at current readiness levels, each additional year of preparation time reduces existential risk by roughly 1-4 percentage points. Given that the expected value of the long-term future is astronomically large, even small reductions in x-risk are enormously valuable.
...
The relationship between preparation time and risk reduction is not linear. It follows an S-curve: very early preparation has modest returns (foundational research), the middle period has the highest returns (translating research into deployable safety measures), and late preparation has diminishing returns (fine-tuning already-adequate measures).
...
Not all acceleration is equal. The concept of <EntityLink id="differential-technology-development">differential technology development</EntityLink> distinguishes between advancing safety-relevant vs. capability-relevant technologies. The ideal is to accelerate safety research while leaving capability timelines unchanged — achieving risk reduction without the costs of delay.

| Development Type | Timeline Effect | Risk Effect | Cost |
|------------------|----------------|-------------|------|
| **Pure safety acceleration** | None | Strong negative (risk reduction) | Low |
| **Pure capability acceleration** | Negative (earlier TAI) | Positive (risk increase) | Delayed benefits cost |
| **Mixed research** (e.g., interpretability) | Slight negative | Net negative (risk reduction) | Low |
| **Infrastructure** (compute, data) | Negative | Positive (risk increase) | Variable |
...
The model implies that capabilities organizations creating acceleration should account for the marginal x-risk created. At current readiness levels, pulling TAI forward by 1 year creates roughly 1-4 percentage points of additional x-risk. If the long-term future is worth \$10^{15}+ in expected value, this represents an enormous externality.
...
| Intervention Type | Effective x-risk-adjusted Years | Cost per Year | Cost-effectiveness Rank |
|-------------------|---------------------------------|---------------|------------------------|
| **Alignment research** | 0.5-3 per breakthrough | \$10-100M per breakthrough | 1st |
| **Compute governance** | 0.3-1.5 globally | \$50-200M for implementation | 2nd |
| **International coordination** | 0.2-1.0 per agreement | \$20-100M per agreement | 3rd |
| **National regulation** | 0.1-0.5 (racing-adjusted) | \$10-50M for advocacy | 4th |
| **Voluntary commitments** | 0.05-0.2 | \$5-20M | 5th |
...
<Backlinks entityId="ai-acceleration-tradeoff" />
