---
wikiId: E637
title: Evaluation Types Table
description: Interactive comparison table of AI evaluation methodologies and their strategic value.
subcategory: analysis-models
contentFormat: table
lastEdited: 2026-02-10
update_frequency: 30
summary: An interactive sortable table comparing 18 AI evaluation approaches across dimensions including signal reliability, coverage depth, Goodhart risk, risk coverage, and scalability. Supports category filtering and grouped/unified views.
clusters:
  - ai-safety
---
import EvalTypesTableView from '@components/tables/views/EvalTypesTableView';

<EvalTypesTableView />
