...
**Limitations of preference learning**: The seminal work on learning from human preferences—reinforcement learning from human feedback, introduced by Christiano, Leike, Brown, Martic, Legg, and Amodei at NeurIPS 2017[^rc-0423]—demonstrated that nuanced behavioral preferences can be learned from non-expert comparisons between pairs of trajectory segments, using feedback from less than 1% of agent interactions. However, this approach has documented limitations:
...
### 2.4 Spontaneous Emergence of Optimization
...
- Current status: As of early 2024, there is little ongoing safety research exploring direct task decomposition, because "it is very hard to break down all problems into simple subtasks"[^rc-b725]

**2. Debate**
- Have two AIs argue; human judges choose the more convincing and truthful argument
- Empirical support: Khan et al. (ICML 2024 Best Paper)[^rc-b026] found that optimizing debaters for persuasiveness improved truth-finding, with judges reaching 76–88% accuracy compared to ~50% baselines. Kenton et al. (NeurIPS 2024)[^rc-eb13] found debate consistently outperforms consultancy across mathematics, coding, logic, and multimodal reasoning tasks.
- Remaining limitations: Requires the human judge to be an adequate evaluator; may not hold when the AI's capability substantially exceeds the judge's. Engels et al. (2025)[^rc-32f1] found that "simply stacking more oversight levels does not guarantee safety if each level is too weak relative to the AI being monitored."
...
**Current status**: Debate shows promising early empirical results, but none of these approaches have been demonstrated to work reliably for AI systems substantially more capable than their human evaluators. Sudhir et al. (2025)[^rc-e0fa] introduced an "agent score difference" metric for evaluating scalable oversight protocols and found that prior results on debate do not straightforwardly generalize across protocols.
...
**Current state**:
- Progress on <EntityLink id="E477" name="mech-interp">mechanistic interpretability</EntityLink> of small models and specific circuits; MLSN #8 (February 2023)[^rc-6606] highlighted work isolating the specific mechanism GPT-2 uses to identify indirect objects in sentences, down to specific parameters
...
**Empirical evidence on proxy optimization scaling**: MLSN #8 (February 2023)[^rc-6606] covered empirical work on scaling laws for proxy gaming in RL-trained language models—finding that over-optimization of proxy rewards can occur both with RL training and with best-of-n sampling, suggesting the problem is not limited to gradient-based optimization.
...
**Misalignment generalization**: Wang et al. (2025)[^rc-ac54] showed that fine-tuning on narrow misalignment spreads to broad misalignment across unrelated domains, and that this process is rapid and does not require large amounts of misaligned training data.
