...
**See also**: <EntityLink id="claim-first-architecture">Claim-First Wiki Architecture</EntityLink> — a companion proposal that inverts the data model, making verified atomic claims (rather than prose documents) the primary artifact, with wiki pages as derived views.
...
This produces pages scoring 70-80/100 on our grading rubric. The pipeline has been iterated significantly (see the <EntityLink id="page-creator-pipeline">Page Creator Pipeline report</EntityLink>) and represents solid work. But it has structural limitations:
...
Recent academic work provides strong validation for the proposition-level and claim-first approaches described in this document and the companion <EntityLink id="claim-first-architecture">Claim-First Architecture</EntityLink> proposal.

### Proposition-Level Retrieval (Dense X Retrieval)

Chen et al. (EMNLP 2024) define a **proposition** as an atomic, self-contained factual expression with three properties: **minimal** (cannot be further split), **self-contained** (includes resolved coreferences), and **compositional** (the union of all propositions reconstructs full semantics). They built a "Propositionizer" model that decomposes text into propositions and showed that indexing Wikipedia at proposition level **significantly outperformed** both passage-level and sentence-level indexing for retrieval and QA tasks.[^11]
...
For AI safety topics where many claims involve genuine disagreement (timelines, risk levels, alignment difficulty), **claim-augmented argumentation frameworks** provide formal tools for representing competing positions. These extend Dung's abstract argumentation by associating a claim to each argument, enabling re-interpretation at different evaluation stages.[^18] This maps to our consensus and analytical claim types and suggests the claim store should eventually support explicit argumentation structure.
...
[^11]: [Dense X Retrieval: What Retrieval Granularity Should We Use?](https://aclanthology.org/2024.emnlp-main.845/) (Chen et al., EMNLP 2024). See also [Factoid Wiki](https://chentong0.github.io/factoid-wiki/).

[^12]: [FActScore: Fine-grained Atomic Evaluation of Factual Precision in Long Form Text Generation](https://arxiv.org/abs/2305.14251) (Min et al., EMNLP 2023)

[^13]: [Nanopublication-based semantic publishing and reviewing](https://peerj.com/articles/cs-1159/) (PeerJ Computer Science, 2023). See also [Nanopublication Guidelines](https://nanopub.net/guidelines/working_draft/).

[^14]: [Micropublications: a semantic model for claims, evidence, arguments and annotations](https://jbiomedsem.biomedcentral.com/articles/10.1186/2041-1480-5-28) (Journal of Biomedical Semantics, 2014)

[^15]: [Knowledge-Centric Templatic Views of Documents](https://arxiv.org/html/2401.06945v1) (arXiv, January 2024)

[^16]: [GPTKB: Building Very Large Knowledge Bases from Language Models](https://arxiv.org/html/2411.04920v1) (arXiv, November 2024). See also [gptkb.org](https://gptkb.org/).

[^17]: [Logseq Architecture — Block-Level Database Design](https://deepwiki.com/logseq/logseq) (DeepWiki)

[^18]: [Claim-augmented argumentation frameworks](https://www.sciencedirect.com/science/article/pii/S000437022300019X) (Artificial Intelligence, 2023)
