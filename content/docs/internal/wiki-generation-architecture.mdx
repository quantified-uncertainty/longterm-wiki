...
- Agent: Enricher
- Cost: \$0.25
- **Status**: The KB system now partially serves this role. Structured facts for 360+ entities exist in `packages/kb/data/things/*.yaml`, with properties defined in `packages/kb/data/properties.yaml`. Pages can reference these via `<KBF>` components and `[^kb-factId]` footnotes. The automated extraction pipeline (proposing new facts from page content) has not been built.
