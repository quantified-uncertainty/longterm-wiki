...
**Partial implementation note**: The `<Calc>` and `<FBF>` (KB Fact) components now partially implement the "computation embedding" vision described here. `<FBF>` pulls live values from KB YAML files in `packages/kb/data/things/`, and `<Calc>` computes derived values from those facts. The Squiggle-based uncertainty modeling described below remains aspirational.
