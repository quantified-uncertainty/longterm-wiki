...
**Lesson for us:** Self-closing `<KBFactValue entity="x" property="y" />` tags are exactly this pattern — an inline query pulling from structured KB data. The wrapping form `<KBFactValue>...$9.7B...</KBFactValue>` is the anti-pattern: it creates TWO sources of truth (KB data and display text).
...
- **Facts** (KB YAML) are the volatile numeric data layer — rendered via `\<KBFactValue>` and `\<Calc>` components at build time, optimized for "update one KB entry, correct 9 pages."
...
**Long-term convergence**: Facts may eventually become a subset of Statements (numeric statements with canonical status), but the KB YAML → `\<KBFactValue>` → build pipeline works well and doesn't need immediate migration.
...
### 1. Most `<KBFactValue>` tags should use self-closing form

The wrapping form duplicates the display text in the MDX:

```mdx
{/* Wrapping form — the number is hardcoded in display text */}
xAI raised <KBFactValue entity="xai" property="total-funding">\$26 billion</KBFactValue> in funding

{/* If KB updates to $30B, display still says "$26 billion" */}
```

This is worse than plain text: it adds JSX complexity without providing the core benefit of single-source-of-truth updates. Prefer the self-closing form: `<KBFactValue entity="xai" property="total-funding" />`.
...
`<KBFactValue entity="ssi" property="headcount">` uses entity slugs and human-readable property names, aligning with KB conventions. `EntityLink` continues to use numeric IDs like `E338` for entity pages.
...
### Self-closing `<KBFactValue>` is the only acceptable inline form

Self-closing `<KBFactValue entity="x" property="y" />` must be the default and strongly-preferred form:

```mdx
{/* Correct — value comes from KB, updates automatically */}
xAI is valued at <KBFactValue entity="xai" property="valuation" /> as of its Series E round.

{/* Wrong — value is hardcoded, defeats the purpose */}
xAI is valued at <KBFactValue entity="xai" property="valuation">\$230 billion</KBFactValue> as of its Series E round.
```
...
The `<KBFactValue>` component requires both `entity` (slug) and `property` (human-readable name):

```mdx
<KBFactValue entity="anthropic" property="valuation" />
<KBFactValue entity="anthropic" property="revenue" asOf="2025-12" />
```

### Display format control

The `<KBFactValue>` component uses KB property display rules for formatting. For historical values, use the `asOf` prop:
...
<KBFactValue entity="anthropic" property="valuation" asOf="2025-11" />
```
...
**Answer: Repurpose.** Kill the wrapping behavior (which produces the 81% low-value tags). Rebuild as a migration tool: scan MDX for hardcoded numbers that match existing KB fact values, and propose replacements with self-closing `<KBFactValue entity="x" property="y" />` tags. This is the inverse of what it does now — instead of adding redundant wrappers, it removes hardcoded values.
...
**Answer:** The `<KBFactValue>` component uses KB property display rules for formatting. The `asOf` prop handles historical values. This eliminates the main reason people use wrapping form (to control display text).
