...
| **FactBase** | Structured triples with temporal data and provenance — facts about entities | YAML files in `packages/factbase/data/fb-entities/` | `apps/web/src/data/factbase.ts` |
...
flowchart TB
    subgraph Sources["Source of Truth"]
        ...
        FB_YAML["packages/factbase/data/fb-entities/*.yaml"]
...
YAML files in `packages/factbase/data/fb-entities/` are currently the primary source. They sync to PG via `crux wiki-server sync-facts`. The PG `facts` table provides an export endpoint (`GET /api/facts/export`) for consumers that need PG-backed access. Once the PG schema includes all Fact fields (`validEnd`, `currency`, etc.), PG will become the primary source.
...
All verification data lives in two tables (replacing the previous six). See [discussion \#2950](https://github.com/quantified-uncertainty/longterm-wiki/discussions/2950).

| PG table | Drizzle export | Purpose |
|----------|---------------|---------|
| `source_check_evidence` | `sourcingEvidence` | Per-source checks for any record type. Supports row-level (`field_name` = NULL) and cell-level (`field_name` = column name) sourcing. |
| `source_check_verdicts` | `sourcingVerdicts` | Aggregate verdicts per claim. Keyed by `(record_type, record_id, COALESCE(field_name, ''))`. |
...
| FactBase `Entity` type | A FactBase thing — an entity in the structured facts system with its own ID scheme | `packages/factbase/data/fb-entities/anthropic.yaml` has `id: mK9pX3rQ7n` |
...
|---------|-------------------|---------|
| `packages/factbase/data/fb-entities/` | FactBase entity YAML files — one file per entity with facts, properties, and metadata | `packages/factbase/data/fb-entities/anthropic.yaml` |
...
**Key distinction**: The FactBase "things" directory (`packages/factbase/data/fb-entities/`) contains YAML data files that define entities and their facts. The PG `things` table is a completely separate concept — it is a universal search/browse index that contains rows pointing to entities, facts, grants, resources, personnel, and other record types. They share a name but serve unrelated purposes.

### "Facts" means different things in different contexts

| Context | What "facts" means | Example |
|---------|-------------------|---------|
| `packages/factbase/data/fb-entities/*.yaml` facts | Structured triples in the FactBase YAML package (the source of truth) | `property: revenue`, `value: 6000000000`, `asOf: 2025-01` |
...
**Key distinction**: The authoritative source for structured facts is the FactBase YAML (`packages/factbase/data/fb-entities/`). The PG `facts` table is a read mirror for API queries. The old `data/facts/*.yaml` system is deprecated for FactBase-covered entities (see <EntityLink id="E1018">Data System Authority Rules</EntityLink>).
