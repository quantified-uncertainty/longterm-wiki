...
The PG `facts` table is the **source of truth** for structured facts. YAML files in `packages/factbase/data/things/` are the editor format — edits are synced to PG via `crux wiki-server sync-facts`, and `build-data.mjs` exports from PG at build time. When wiki-server is unavailable (local dev), YAML serves as a fallback.

| PG table | Drizzle export | Purpose |
|----------|---------------|---------|
| `facts` | `facts` | **Source of truth** for FactBase facts. Numeric/string facts with timeseries support via `measure` + `as_of`. |
| `properties` | `properties` | Controlled vocabulary for fact property types (valuation, headcount, ceo, etc.). |
| ~~`factbase_resource_verifications`~~ | — | Replaced by unified `verification_evidence` table (migration 0127). |
| ~~`factbase_verdicts`~~ | — | Replaced by unified `verification_verdicts` table (migration 0127). |
...
| PG table | Drizzle export | Purpose |
|----------|---------------|---------|
| `verification_evidence` | `verificationEvidence` | Per-source checks for any record type. Supports row-level (`field_name` = NULL) and cell-level (`field_name` = column name) verification. |
| `verification_verdicts` | `verificationVerdicts` | Aggregate verdicts per claim. Keyed by `(record_type, record_id, COALESCE(field_name, ''))`. |
...
| ~~`record_verifications`~~ | — | Replaced by unified `verification_evidence` table (migration 0127). |
| ~~`record_verdicts`~~ | — | Replaced by unified `verification_verdicts` table (migration 0127). |
