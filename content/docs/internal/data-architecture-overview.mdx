...
| `citation_content` | Latest HTML/text from cited URLs (hot cache) | Fetched on-demand during source-checks and citation verification |
...
Putting it all together — here's the **auto-update path** (the most common flow) from a real-world event to a reader seeing updated information. Other paths exist: TableBase enrichment (scan → rank → agent → pre-submit verify → PG), on-demand source-checks, and manual edits.
...
| **Source-check coverage is partial** | Only about 15% of wiki pages have been source-checked. Many facts lack source URLs entirely | `crux fb source-check` and `crux w source-check-wiki-pages` are available but expensive (about \$0.07/page) |
