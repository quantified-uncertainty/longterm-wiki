...
description: "High-level overview of the wiki's layered data architecture — from raw schema through scraping, sourcing, and the three derived data bases. Includes diagrams, tables, and a guide to common naming confusions."
...
|-------|---------------|-------------------|
| `resources` | Metadata for external papers, blog posts, reports | Build-data sync from YAML + API discovery |
| `citation_content` | Latest HTML/text from cited URLs | Fetched on-demand during source-checks and citation verification |
| `resource_content_versions` | **Versioned** content snapshots with content-hash dedup | Fetched by source-check pipeline and website monitors |
| `website_source_pages` | Monitored pages from tracked websites | Configured per website source (e.g., org career pages, grant databases) |
...
| Academic | 3 | RSS | arXiv cs.AI, cs.CL, cs.LG |
| Policy / Governance | 3 | Web-search | AI legislation, executive orders, policy news |
| News Aggregators | 5 | RSS + web-search | AI safety newsletters, The Decoder, etc. |
| Other | 7 | Various | YouTube channels, podcast feeds, govt sources |
...
| `crux/lib/source-check/source-fetcher.ts` | Fetches and caches source content (Firecrawl, HTTP, YouTube) |
...
Each verdict carries a **confidence score** (0.0--1.0) and is timestamped so stale checks can be detected.

### CLI

```bash
# FactBase facts
pnpm crux fb source-check --entity=anthropic      # Check all facts for an entity
pnpm crux fb source-check --fact=f_dW5cR9mJ8q     # Check a single fact

# Wiki page claims
pnpm crux w source-check-wiki-pages --page=anthropic        # Check one page
pnpm crux w source-check-wiki-pages --limit=5 --budget=2    # Batch with budget
```
...
| File | Role |
|------|------|
| `crux/lib/source-check/orchestrator.ts` | Main orchestration logic |
| `crux/lib/source-check/item-verifier.ts` | LLM-based claim verification |
| `crux/lib/source-check/deterministic-matcher.ts` | Cross-references claims to footnotes |
| `crux/lib/source-check/verdict-handler.ts` | Stores evidence and computes aggregate verdicts |
| `crux/lib/source-check/wiki-page-claims.ts` | Extracts claims from MDX prose via LLM |
...
|------|---------------|----------------|---------------|--------------|
| **TableBase** | Typed entity catalog (orgs, people, models, risks, concepts) | `data/entities/*.yaml` (15 files) | `database.json` | `crux tb` |
| **FactBase** | Structured temporal facts with provenance | `packages/factbase/data/things/*.yaml` (539 files) | `factbase-data.json` | `crux fb` |
| **WikiBase** | Long-form prose articles | `content/docs/**/*.mdx` (≈730 pages) | `database.json` (pages section) | `crux w` |
...
Most data entry is automated. The goal is a **defensive pipeline**: data enters through verification gates so records are born with green source-check dots, rather than being verified after the fact. See [Discussion \#3958](https://github.com/quantified-uncertainty/longterm-wiki/discussions/3958) for the full strategy.
...
|--------|-------------|------------|
| **V1** (fixed pipeline) | Sequential phases: research → generate → review. Deterministic order. | Single-page creation/improvement, `crux w create` |
...
4. **Pre-submit verification** checks each record against its source URL before writing — records are born with verdicts (the "defensive pipeline" from [Discussion \#3958](https://github.com/quantified-uncertainty/longterm-wiki/discussions/3958))
...
The improve pipeline extracts some facts as a side effect (wrapping claims in `\<FBFactValue\>` tags), but there is no dedicated automated fact discovery loop yet.
...
| FactBase `Entity` type | FactBase thing with facts | 10-char alphanumeric | `packages/factbase/data/things/anthropic.yaml` |
...
|---------|-------------------|---------|
| `packages/factbase/data/things/` | FactBase entity YAML files | One file per entity, containing facts and metadata |
...
|---------|-------------------|--------|
| `packages/factbase/data/things/*.yaml` | FactBase structured triples (authoritative) | Active, primary |
...
| **Three Bases** | 94 validators in `crux/validate/`, build phase validation | Gate checks (6 CI-blocking), build-time error detection | <EntityLink id="E908">Entities</EntityLink>, <EntityLink id="E909">Page Changes</EntityLink>, <EntityLink id="E900">Update Schedule</EntityLink>, <EntityLink id="E1054">DB Schema</EntityLink> |

### CI-Blocking Gate Checks

Six validators are CI-blocking (code cannot merge if they fail):

1. **comparison-operators** — MDX `<` must be escaped as `\<`
2. **dollar-signs** — MDX `$` must be escaped as `\$`
3. **schema** — Entity YAML must match type schema
4. **frontmatter-schema** — MDX frontmatter must be valid
5. **wiki-id-integrity** — No duplicate or missing wikiIds
6. **prefer-entitylink** — Use `<EntityLink>` not raw markdown links for entities

Run all: `pnpm crux w validate gate --fix`

---

## How Data Flows End-to-End

Putting it all together — from a real-world event to a reader seeing updated information:
...
flowchart TB
    ...
    subgraph SourceCheck["Source-Check Layer"]
...
| **Internal dashboards** | Wiki-server API via Hono RPC | Runtime (ISR, 300s cache) | Grants, personnel, source-checks, jobs |
...
| **Add structured facts about an entity** | `pnpm crux fb add-fact` or edit `packages/factbase/data/things/<entity>.yaml` directly | `packages/factbase/data/things/` |
| **Check if a fact is accurately sourced** | `WIKI_SERVER_ENV=prod pnpm crux fb source-check --entity=anthropic` | `crux/lib/source-check/` |
| **Find why a page shows stale data** | Check `update_frequency` in frontmatter → `WIKI_SERVER_ENV=prod pnpm crux w auto-update history` → check source-check verdicts | `data/auto-update/sources.yaml` |
...
|-------|----------------|---------|
| **`w` (wiki)** | `create`, `improve`, `validate gate`, `fix escaping`, `auto-update`, `source-check-wiki-pages`, `citations`, `qa-sweep` | Content authoring, validation, automated updates |
| **`fb` (factbase)** | `show`, `validate`, `search`, `add-fact`, `source-check`, `coverage` | Structured fact management and verification |
| **`tb` (tablebase)** | `ids allocate`, `tablebase scan/gaps/improve/loop`, `ensure-entities`, `people discover`, `benchmarks`, `source-check` | Entity catalog, PG table enrichment, ID allocation |
...
| **Source-check coverage is partial** | Only ≈15% of wiki pages have been source-checked. Many facts lack source URLs entirely | `crux fb source-check` and `crux w source-check-wiki-pages` are available but expensive (≈\$0.07/page) |
