...
Unified verification system for all record types (facts, grants, personnel, divisions, etc.). See [discussion \#2950](https://github.com/quantified-uncertainty/longterm-wiki/discussions/2950).
...
erDiagram
    ...
    resources ||--o{ verification_evidence : "checked against"
    verification_verdicts ||--o{ verification_evidence : "derived from"
`} />

The verification system has two layers:
1. **Evidence** (`verification_evidence`): One row per source-claim check. Records what the LLM found. Supports row-level (`field_name` = NULL) and cell-level (`field_name` = column name) verification.
2. **Verdicts** (`verification_verdicts`): One row per claim. Aggregated verdict derived from all evidence rows. Verdict options: confirmed, contradicted, unverifiable, outdated, partial, unchecked.
