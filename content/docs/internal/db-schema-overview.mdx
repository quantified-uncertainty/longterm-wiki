...
erDiagram
    ...
    resources ||--o{ kb_source_checks : "checked against"
    kb_fact_verdicts ||--o{ kb_source_checks : "derived from"
`} />

The verification system has two layers:
1. **Evidence** (`kb_source_checks`): One row per fact-resource check. Records what the LLM found.
