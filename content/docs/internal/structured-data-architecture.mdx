...
thing:
  ...
  name: Anthropic
  wikiId: "E22"       # Legacy wiki page ID
...
### `<FBF>` \- Inline Fact Display

Renders a single KB fact with a hover tooltip showing source, date, and notes:

```jsx
<FBF entity="anthropic" property="valuation" />          {/* Auto-formatted: "$380.0B" */}
<FBF entity="anthropic" property="revenue" showDate />   {/* "$19.0B (as of 2026-03)" */}
<FBF entity="anthropic" property="valuation">$380B</FBF> {/* Custom display text */}
```

### `<FBFactValue>` \- Block Fact Display
...
Facts referenced via `<FBF>` automatically generate footnote citations using the `[^kb-factId]` format. These appear in the page's footnote section with full source attribution.
...
| **Display** | `<FBF>`, `<Calc>`, tooltips | Statements table on entity pages |
...
| `\<FBF\>` inline fact display with tooltips | Working |
| `\<FBFactValue\>` block display | Working |
...
- **Temporal queries in components**: Enabling `\<FBF\>` to reference facts at specific dates (e.g., "Anthropic's revenue as of 2025-03") rather than always showing the latest
