...
Mirrored files live at `packages/factbase/data/fb-entities/<slug>/scorecard-grades.yaml`
with a "do not edit by hand" header — re-running the sync rewrites them
in place. When an entity has no other per-entity files in its directory,
removing the scorecard file folds the directory back to the original
`<slug>.yaml` form.
