...
<F e="anthropic" f="7a3815b4" showDate />

{/* Computed value from two facts */}
<Calc expr="{anthropic.6796e194} / {anthropic.0ed4db9e}" precision={0} suffix="x" />

{/* Growth percentage */}
<Calc expr="({anthropic.83e76962} / {anthropic.bc497a3d} - 1) * 100" precision={0} suffix="%" />
```
...
<F e="anthropic" f="6796e194">\$380 billion</F>

{/* Auto-display from the fact's value field */}
<F e="anthropic" f="6796e194" />

{/* With inline date for temporal claims */}
Over <F e="anthropic" f="7a3815b4" showDate /> business customers
...
{/* Comparison */}
<Calc expr="{openai.39d6868e} / {anthropic.6796e194}" precision={1} suffix="x" />
```
...
<Calc expr="{anthropic.6796e194}" format="currency" />
...
Facts live in `data/facts/<entity>.yaml`. Each fact has an **8-character hex hash ID** (not a human-readable name). A comment above each fact shows its purpose:
...
facts:
  # valuation
  6796e194:
    value: 380e9                   # Structured numeric (build auto-formats)
    asOf: "2026-02"                # When this was current
    measure: valuation             # Links to fact-measures.yaml
    note: "Series G post-money"    # Context shown in tooltip
    source: "https://..."          # Attribution URL
```

**Why hash IDs?** Naming facts is tedious and error-prone at scale. Hash IDs are generated once (`generateFactId()` from `crux/resource-utils.ts`) and never need to change. The YAML comment and `measure` field provide human context.

**Required fields:**
- `value` — numeric (e.g. `380e9`, `[20e9, 26e9]`, `\{min: 300000\}`) or string (e.g. `"$76,001/year"`, `"2028"`). Numeric values are auto-formatted at build time.
- `asOf` — temporal anchor (strongly recommended for any changing number)
- `measure` — must reference a valid ID from `data/fact-measures.yaml` (required for timeseries grouping)

**Optional fields:**
- `note` — shown in tooltip
- `source` — attribution link
- `sourceResource` — 16-char resource hash ID linking to `data/resources/*.yaml`
...
1. Generate an ID: use `generateFactId()` or run `node -e "console.log(require('crypto').randomBytes(4).toString('hex'))"`
2. Add the entry to the appropriate `data/facts/<entity>.yaml` file with a comment
...
```jsx
valued at <F e="anthropic" f="6796e194" /> in its latest funding round
```
...
| Metric | Value |
|--------|-------|
| **Revenue Multiple** | <Calc expr="{anthropic.6796e194} / {anthropic.0ed4db9e}" precision={0} suffix="x" /> |
```

### Growth rate comparison

```jsx
revenue grew <Calc expr="({anthropic.83e76962} / {anthropic.bc497a3d} - 1) * 100" precision={0} suffix="%" /> in seven months
```

### Side-by-side company comparison

```jsx
Anthropic trades at ≈<Calc expr="{anthropic.6796e194} / {anthropic.0ed4db9e}" precision={0} suffix="x" /> revenue
vs OpenAI's ≈<Calc expr="{openai.39d6868e} / {openai.609b9796}" precision={0} suffix="x" />
```
