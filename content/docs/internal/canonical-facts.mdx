...
- **`<KBF>`** — display a single KB fact inline (with hover tooltip showing metadata)
...
3. Reference in MDX with `<KBF entity="entity" property="property" />`
...
4. Every `<KBF>` and `<Calc>` referencing that property auto-updates
