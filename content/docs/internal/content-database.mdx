...
KB facts are rendered on wiki pages via `<KBF>` and `<KBFactValue>` components, and computed values via `<Calc>`. See <EntityLink id="E1018" name="data-system-authority">Data System Authority Rules</EntityLink> for which system is authoritative for which entities.
