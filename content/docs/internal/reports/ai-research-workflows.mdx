...
evergreen: false
summary: "Internal guide recommending a multi-phase AI research pipeline (context assembly → Opus planning → Perplexity research → Sonnet drafting → validation) over single-shot prompts, with cost estimates of \\$2.65-5.20 per article. Provides comprehensive survey of 2025-2026 research APIs (Perplexity, Exa, Tavily, Elicit, Scry) with pricing and feature comparisons."
---
...
| **Deep Research APIs exist** | Perplexity Sonar, <EntityLink id="E218" name="openai">OpenAI</EntityLink> Deep Research, Gemini Deep Research | Consider OpenRouter for Perplexity API access |
...
```bash
pnpm crux validate compile        # Syntax errors
pnpm crux validate unified --rules=dollar-signs,comparison-operators
pnpm crux validate entity-links   # Broken links
```
...
```bash
# .env additions
ANTHROPIC_BILLING_KEY=sk-ant-...  # For Claude Code SDK
