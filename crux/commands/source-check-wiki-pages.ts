 ...
 *   crux w source-check-wiki-pages --limit=5 --budget=2     Check up to 5 pages, $2 budget
 *   crux w source-check-wiki-pages --page=anthropic         Check a single page
 *   crux w source-check-wiki-pages --min-importance=70      Only high-importance pages
 */
...
const LOG_PREFIX = '[source-check-wiki]';
...
async function sourceCheckWikiPagesCommand(
  _args: string[],
...
export const commands = {
  default: sourceCheckWikiPagesCommand,
};
...
Usage:
  crux w source-check-wiki-pages [options]             Check top pages by importance
  crux w source-check-wiki-pages --page=anthropic      Check a specific page
  crux w source-check-wiki-pages --dry-run             Preview what would be checked
