/**
 * Tests for sourcing orchestrator routing — verifies that record-type
...
vi.mock('../../lib/sourcing/orchestrator.ts', () => ({
  orchestrateCommand: (...args: unknown[]) => mockOrchestrateCommand(...args),
}));

// Mock getSourcingStats (used by stats subcommand)
vi.mock('../../lib/wiki-server/sourcing-client.ts', () => ({
  getSourcingStats: vi.fn().mockResolvedValue({
    ok: true,
...
const { commands } = await import('../sourcing-orchestrate.ts');
