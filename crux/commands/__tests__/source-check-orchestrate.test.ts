/**
 * Tests for source-check orchestrator's Zod schema parsing and verdict validation.
 *
 * Tests the LlmResponseSchema (batch JSON parsing) and validateVerdict function
 * from crux/lib/source-check/llm-checker.ts — the core parsing logic used by
 * the orchestrator when processing LLM responses.
 */
import { describe, it, expect } from 'vitest';
import { LlmResponseSchema, validateVerdict } from '../../lib/source-check/llm-checker.ts';
...
describe('LlmResponseSchema', () => {
  ...
  describe('verdict field accepts any string (validated downstream)', () => {
    ...
    // should parse without throwing, and callLlmForSourceCheck maps unknown
...
describe('LlmResponseSchema + validateVerdict integration', () => {
  it('schema parses then validateVerdict filters unknown verdicts', () => {
    // Simulates the flow in callLlmForSourceCheck: parse with Zod, then validate verdict
    const raw = {
