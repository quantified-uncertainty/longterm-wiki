/**
 * Tests for source-check-filter.ts
 *
 * Focus: the filterBySourceCheckVerdicts function correctly skips pages
...
import { filterBySourceCheckVerdicts, type SourceCheckFilterDeps } from './source-check-filter.ts';
...
function makeDeps(overrides: Partial<SourceCheckFilterDeps> = {}): SourceCheckFilterDeps {
  return {
...
describe('filterBySourceCheckVerdicts', () => {
  it('returns empty result for empty input', async () => {
    const getIds = vi.fn();
    const result = await filterBySourceCheckVerdicts([], false, makeDeps({ getContradictedEntityIds: getIds }));
  ...
  it('passes all pages when no contradicted verdicts exist', async () => {
    ...
    const result = await filterBySourceCheckVerdicts(updates, false, deps);
  ...
  it('does not mutate the input array', async () => {
    ...
    await filterBySourceCheckVerdicts(updates, false, deps);
  ...
  it('logs verbose details when verbose=true and pages are skipped', async () => {
    ...
    await filterBySourceCheckVerdicts(updates, true, deps);
