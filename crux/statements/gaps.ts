...
// ---------------------------------------------------------------------------
// Exported: core gap analysis (used by improve.ts)
// ---------------------------------------------------------------------------
...
/**
 * Analyze coverage gaps for an entity. Reusable core — no CLI, no console output.
 */
export async function analyzeGaps(entityId: string, orgType?: string | null): Promise<GapAnalysis> {
  ...
  ] as GapAnalysis['allStatements'];

  const categoryCounts: Record<string, number> = {};
  for (const stmt of allStatements) {
    if (stmt.status === 'retracted' || stmt.status === 'superseded') continue;
  ...
  return {
    entityType,
    totalStatements: allStatements.filter((s) => s.status !== 'retracted' && s.status !== 'superseded').length,
    coverageScore,
    gaps,
    categoryCounts,
    allStatements,
