/**
 * Tests for claim-sourcing handler — LLM output validation logic.
 *
 * Covers:
...
import { handleClaimSourcing } from '../claim-sourcing.ts';
...
/** Build a JSON LLM response string from claim sourcing results. */
...
describe('handleClaimSource-Check — LLM output validation', () => {
  it('skips unknown claimIds and marks expected claims as unverifiable', async () => {
    ...
    const result = await handleClaimSourcing(makeParams([1, 2]), CTX);
  ...
  it('skips duplicate claimIds (keeps first occurrence only)', async () => {
    ...
    const result = await handleClaimSourcing(makeParams([10, 20]), CTX);
  ...
  it('marks omitted claims as unverifiable with error entry', async () => {
    ...
    const result = await handleClaimSourcing(makeParams([100, 200, 300]), CTX);
  ...
  it('returns failure when evidence storage throws (issue #4017)', async () => {
    ...
    // First storeSourcingEvidence call rejects, second succeeds.
    mockStoreSourcingEvidence
    ...
    const result = await handleClaimSourcing(makeParams([7, 8]), CTX);
    warnSpy.mockRestore();
    ...
    expect(mockStoreSourcingEvidence).toHaveBeenCalledTimes(2);
  });
  ...
  it('marks job failed when ALL evidence storage calls throw (issue #4017)', async () => {
    ...
    mockStoreSourcingEvidence.mockRejectedValue(new Error('connection refused'));
    ...
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const result = await handleClaimSourcing(makeParams([11]), CTX);
    warnSpy.mockRestore();
  ...
  it('returns failure when verdict write fails', async () => {
    ...
    const result = await handleClaimSourcing(makeParams([5]), CTX);
