/**
 * Red-team / adversarial tests for the claim sourcing handler.
 *
...
vi.mock("../../sourcing/verdict-handler.ts", () => ({
  storeSourcingEvidence: vi.fn(() => Promise.resolve()),
}));
...
const { handleClaimSourcing } = await import("../claim-sourcing.ts");
...
describe("handleClaimSource-Check — adversarial inputs", () => {
  ...
  it("rejects params with missing claimIds", async () => {
    const result = await handleClaimSourcing(
  ...
  it("handles all claims failing sourcing (100% error rate)", async () => {
