/**
 * Source Fetcher — read cached source documents for sourcing.
 *
 * Shared by factbase-sourcing and sourcing-orchestrate. This is a
...
/**
 * Read cached source content for sourcing.
 *
 ...
 * @param logPrefix - Prefix for log messages (default: '[sourcing]')
 */
export async function fetchSourceContent(
  url: string,
  _userAgent = 'LongtermWiki-Sourcinger/1.0',
  logPrefix = '[sourcing]',
  ...
  if (isDeadLink && waybackEnabled) {
    ...
    try {
      ...
      // Best-effort: Wayback failure should not block the sourcing pipeline
  ...
  //
  // Fire-and-forget: don't block the caller or fail the sourcing.
  ...
  try {
    ...
    if (resourceId) {
      const jobResult = await createJob({
        ...
        priority: 1, // Slightly elevated — sourcing is actively waiting for this
    ...
    // Best-effort — don't fail sourcing if enqueue fails
