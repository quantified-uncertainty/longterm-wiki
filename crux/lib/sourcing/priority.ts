/**
 * Priority computation for sourcing items.
...
import { ENTITY_TYPE_PRIORITY, type SourcingedFactInfo, type SourcingedRecordInfo } from './orchestrator-types.ts';

export function computeFactPriority(
  entity: FBEntity,
  _fact: Fact,
  existing: SourcingedFactInfo | undefined,
  ...
  if (!existing) {
    ...
    // Staleness: older sourcing get higher priority
...
export function computeRecordPriority(
  ...
  existing: SourcingedRecordInfo | undefined,
): number {
  let priority = 0;
