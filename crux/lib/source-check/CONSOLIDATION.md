...
### 1. `factbase-source-check` (`crux/commands/factbase-source-check.ts`)
- **Scope**: FactBase facts (structured triples with source URLs)
- **Entry point**: `crux fb source-check --entity=X`
...
### 3. `source-check-wiki-pages` (`crux/commands/source-check-wiki-pages.ts`)
- **Scope**: Wiki page prose — extracts factual claims from MDX content and verifies against footnoted sources
- **Entry point**: `crux w source-check-wiki-pages --page=X`
...
### Adjacent: `tablebase/source-check.ts` (`crux/tablebase/source-check.ts`)
...
| LLM verification call | own impl | shared `callLlmForSourceCheck` | own impl | own impl |
| Evidence storage | shared `storeSourceCheckEvidence` | shared | shared | shared |
...
**Shared utilities** already extracted to `crux/lib/source-check/`:
- `source-fetcher.ts` — URL fetching with paywall detection, caching, HTML-to-text
- `llm-checker.ts` — `callLlmForSourceCheck()`, verdict validation
- `verdict-handler.ts` — `storeSourceCheckEvidence()`, `storeAggregateVerdict()`
...
## Recommendation: `source-check-orchestrate` as canonical
...
### Phase 1: Absorb `factbase-source-check` into orchestrator
- The orchestrator already handles `--type=fact`. The standalone command adds no unique functionality.
- **Action**: Deprecate `crux fb source-check` with a message pointing to `crux source-check orchestrate --type=fact`.
...
- `factbase-source-check` and `source-check-wiki-pages` each have their own LLM prompt construction.
- **Action**: Migrate remaining callers to `callLlmForSourceCheck()` from `llm-checker.ts`, extending it with a `mode` parameter if prompt variations are needed.

### Phase 4: Batch API integration
- `tablebase/source-check.ts` uses the Batch API for cost savings.
...
All core utilities are already in `crux/lib/source-check/`:
- `fetchSourceContent` — content fetching with cache and paywall detection
- `callLlmForSourceCheck` — LLM verification with structured response parsing
- `storeSourceCheckEvidence` — evidence persistence
