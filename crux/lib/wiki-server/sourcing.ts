 ...
 *
 * Provides functions to fetch sourcing verdicts from the wiki-server.
 * Used by the auto-update pipeline to skip pages with contradicted verdicts,
 * and by agents to query sourcing status by entity, page, or failure type.
 *
...
/**
 * Fetch sourcing verdicts filtered by verdict type.
 */
...
/**
 * Fetch all contradicted sourcing verdicts and return the set of
...
export async function getContradictedEntityIds(): Promise<Set<string>> {
  ...
  if (!result.ok) {
    console.warn(`[sourcing] Failed to fetch contradicted verdicts: ${result.message}`);
    return new Set();
  }

  const { verdicts, total } = result.data;
  if (total > verdicts.length) {
    console.warn(`[sourcing] Fetched ${verdicts.length}/${total} contradicted verdicts`);
  }
