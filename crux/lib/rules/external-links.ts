 ...
 */
function checkUrl(url: string): Promise<{ status: number; ok: boolean; error?: string }> {
 ...
 */
async function checkUrlsWithConcurrency(urls: string[], cache: LinkCache): Promise<LinkCache> {
  const results: LinkCache = {};
  ...
  async function worker(): Promise<void> {
    while (index < unchecked.length) {
      ...
      try {
        ...
        results[url] = { status: 0, ok: false, error: message, checkedAt: Date.now() };
      }
    }
  }
 ...
 */
function extractExternalUrls(body: string): Array<{ url: string; line: number; text: string }> {
...
export const externalLinksRule = createRule({
  ...
  async check(allContent: ContentFile[], engine: ValidationEngine): Promise<Issue[]> {
    const issues: Issue[] = [];

    // Collect all external URLs with their source locations
    const urlLocations = new Map<string, Array<{ file: string; line: number; text: string }>>(); // url → [{file, line, text}]
    ...
    const mergedCache: LinkCache = { ...cache, ...freshResults };
