...
export async function runEnrichmentAgent(
  ...
  const { dryRun = false, model = MODELS.sonnet, skipVerification = false, apply = false } = options;
  ...
  const toolHandlers = buildToolHandlers(task, dryRun, { skipVerification, apply });
