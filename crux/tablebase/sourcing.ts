 ...
 *   2. LLM sourcing via Batch API: cross-reference record claims against source content
 *
 * Usage:
 *   crux tb sourcing-records --table=personnel           # Check all personnel records
 *   crux tb sourcing-records --table=personnel --limit=50 # Check first 50
 *   crux tb sourcing-records --source=batch               # LLM sourcing via Batch API
 *   crux tb sourcing-records --source=deterministic       # Structural checks only (no LLM)
 */
 ...
 * because they are redundant with resolved names or not useful for sourcing.
 */
...
/**
 * Create a human-readable copy of a record for LLM sourcing.
 *
 ...
 * - Strips internal-only fields that don't help sourcing
...
export function runDeterministicChecks(
  ...
  for (const rec of records) {
    ...
    // (handled by dedup.ts on submission, but good to flag in sourcing too)
  }
...
/** Build a sourcing prompt for a single record */
...
/** Parse the LLM sourcing response */
function parseSourcingResponse(
  text: string,
  recordId: string,
  table: string,
): SourcingIssue[] {
  ...
  try {
    ...
    issues.push({
      ...
      field: '_parse',
      severity: 'info',
      message: `Could not parse LLM sourcing response`,
      source: 'llm',
    });
  }
...
/**
 * Run LLM sourcing on records via the Batch API.
...
export async function runBatchSourcing(
...
): Promise<{ issues: SourcingIssue[]; cost: number }> {
  ...
  const batchRequests: BatchRequest[] = records.map((rec) => {
    ...
    return {
      ...
      params: {
        ...
        max_tokens: 1024,
        messages: [
          { role: 'user' as const, content: buildSourcingPrompt(table, rec, nameMap) },
        ],
      },
    };
  });
  ...
  console.log(`[verify] Submitting batch of ${batchRequests.length} sourcing requests (model: ${model})...`);
  ...
  for (const [customId, result] of results) {
    ...
    if (text) {
      const recordIssues = parseSourcingResponse(text, recordId, table);
...
export interface VerifyOptions {
  ...
  /** LLM model for batch sourcing (default: haiku) */
  model?: string;
  /** Max records per batch request (Batch API limit is 10,000) */
  batchSize?: number;
}

export async function verifyRecords(options: VerifyOptions): Promise<SourcingResult> {
  ...
  if (source === 'batch' || source === 'all') {
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      const { issues: batchIssues, cost } = await runBatchSourcing(table, chunk, { model });
...
export function formatSourcingReport(result: SourcingResult): string {
  const lines: string[] = [];
