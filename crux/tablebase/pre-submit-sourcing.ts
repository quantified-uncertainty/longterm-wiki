 ...
 *
 * Records that fail sourcing are excluded from submission with a warning.
 */
...
export interface SourcingedRecord {
  record: Record<string, unknown>;
  /** Whether the record passed the sourcing and should be submitted */
  passed: boolean;
...
export interface PreSubmitResult {
  /** Records that passed the sourcing (include in submission) */
  accepted: Array<Record<string, unknown>>;
  /** Records that failed the sourcing (excluded from submission) */
  rejected: SourcingedRecord[];
  /** Records that were skipped (no source URL) — included without sourcing */
  noSource: Array<Record<string, unknown>>;
  /** Total sourcing cost in USD */
  cost: number;
}
...
/**
 * Build a sourcing prompt for a personnel/enrichment record.
 ...
 *   2. Call the LLM sourcinger (Haiku) to compare record vs source
 *   3. If verdict is "contradicted" or "unverifiable" with low confidence: reject
 *   4. Otherwise: accept with sourcing metadata attached
 *
 ...
 * @param records - Records to sourcing
...
export async function verifyBeforeSubmit(
  ...
  const rejected: SourcingedRecord[] = [];
  ...
  for (const record of records) {
    ...
    if (!sourceUrl) {
      console.warn(`[pre-verify] Record ${record.id ?? '?'}: no source URL — skipping sourcing`);
    ...
    if (!fetchResult.content) {
      ...
      console.warn(`[pre-verify] Record ${record.id ?? '?'}: source content unavailable (${reason}) — including without sourcing`);
    ...
    try {
      const result = await callLlmForSourcing(client, prompt, label);
      ...
      // Record passed sourcing — attach sourcing data
      record.sourcing = sourcing;
      accepted.push(record);
    } catch (err: unknown) {
      // LLM call failed — include the record but log the error
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(`[pre-verify] Record ${record.id ?? '?'}: sourcing failed (${msg}) — including without sourcing`);
      accepted.push(record);
    }
  }
...
/**
 * Format a human-readable summary of the pre-submit sourcing results.
 */
