...
export async function generatePolicyStakeholdersSnapshot(options?: {
  ...
  while (true) {
    ...
    const rows = (data.policyStakeholders || []) as PGPolicyStakeholderRow[];
    if (rows.length === 0) break;
    allStakeholders.push(...rows);
    offset += rows.length;
  ...
  // Sort deterministically so daily snapshot diffs stay reviewable when
  // committed back to main. (id is the natural-key hash — stable across runs.)
  const sorted = [...allStakeholders].sort((a, b) => a.id.localeCompare(b.id));
...
async function main() {
  ...
  await generatePolicyStakeholdersSnapshot({ dryRun });
}
