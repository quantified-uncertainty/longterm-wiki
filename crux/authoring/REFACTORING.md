...
Split from ~2,400 lines into modules under `crux/authoring/creator/`:
...
Replaced inline implementation with delegation to shared `componentImportsRule` from `crux/lib/rules/component-imports.mjs`.

### grade-content.mjs — duplicated utilities ✅
- `extractFrontmatter()` → delegates to `parseFrontmatter()` from `crux/lib/mdx-utils.mjs`
- `collectPages()` → uses `findMdxFiles()` from `crux/lib/file-utils.mjs`
...
- `countWords()`, `countTables()` → imported from `crux/lib/metrics-extractor.mjs` (newly exported)
...
Updated `crux/lib/rules/component-imports.mjs` to match actual exports from `app/src/components/wiki/index.ts`.
...
Added test files:
- `crux/authoring/creator/creator.test.mjs` — 21 tests for duplicate detection and URL extraction
- `crux/lib/metrics-extractor.test.mjs` — 23 tests for exported metric functions
...
- **grade-by-template.mjs — PAGE_TEMPLATES inline data**: Extracted to `crux/lib/page-templates.mjs` so both grading and authoring tools can share them.
