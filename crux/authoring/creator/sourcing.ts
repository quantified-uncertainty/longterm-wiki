...
export async function runSourcing(topic: string, { log, saveResult, getTopicDir }: SourcingContext): Promise<{ success: boolean; warnings: Warning[] }> {
  ...
  if (!fs.existsSync(researchPath) || !fs.existsSync(draftPath)) {
    log('verify-sources', 'Missing research or draft, skipping sourcing');
  ...
  if (fetchedContent) {
    log('verify-sources', `Using ${fetchedContent.sourceCount} fetched sources for sourcing (${Math.round(fetchedContent.combinedContent.length / 1000)}k chars)`);
  } else {
