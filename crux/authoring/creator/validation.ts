...
5. **Fix markdown list formatting**:
   ...
   - Check with: \`node crux/crux.mjs validate unified --rules=markdown-lists\`

6. **Fix consecutive bold labels**:
   - Bold lines like "**Label:** text" need blank line between them
   - Check with: \`node crux/crux.mjs validate unified --rules=consecutive-bold-labels\`
...
export async function runFullValidation(topic: string, { log, saveResult, ROOT, getTopicDir }: FullValidationContext): Promise<{ success: boolean; results: ValidationResults }> {
  ...
  try {
    ...
    execSync('node crux/crux.mjs validate compile --quick', {
      cwd: ROOT,
      stdio: 'pipe',
      timeout: 60000
    });
      ...
      if (json) {
        ...
        try {
          const grepOutput = execSync(
            `node crux/crux.mjs validate unified --rules=${rule} 2>&1 | grep -i "${topicSlug}" || true`,
            { cwd: ROOT, stdio: 'pipe', timeout: 30000, maxBuffer: 10 * 1024 * 1024 }
          ).toString();
