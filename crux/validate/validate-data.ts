...
export function runCheck(options: ValidatorOptions = {}): ValidatorResult {
  ...
  // Build set of KB thing slugs from packages/factbase/data/fb-entities/
  const kbThingsDir = join(PROJECT_ROOT, 'packages/factbase/data/fb-entities');
  ...
  for (const expert of experts) {
    ...
    if (!kbThingSlugs.has(expert.id)) {
      console.log(`${colors.yellow}⚠️  Expert "${expert.id}" has no matching KB thing file at packages/factbase/data/fb-entities/${expert.id}.yaml${colors.reset}`);
      warnings++;
      expertMismatches++;
    }
  }
