 ...
 *
 * Usage: node scripts/validate/validate-financials.ts [--ci]
 *
 * Exit codes:
 *   0 = No issues found
 *   1 = Issues found
...
const FINANCIAL_PATH_PATTERNS: RegExp[] = [
  /\/people\//,
  /\/funders\//,
];

// Known major assets that should be cross-referenced
const MAJOR_ASSETS: AssetConfig[] = [
 ...
 */
function isFinancialPage(filePath: string): boolean {
  return FINANCIAL_PATH_PATTERNS.some((p: RegExp) => p.test(filePath));
}

/**
 * Extract financial figure contexts from page content
 */
function extractFinancialContexts(body: string): FinancialContext[] {
 ...
 */
function checkFinancialPage(filePath: string, frontmatter: Record<string, unknown>, body: string): FinancialIssue[] {
  ...
  for (const ctx of contexts) {
    const oldYears = ctx.years.filter((y: number) => y <= staleThreshold);
    if (oldYears.length > 0 && !ctx.years.some((y: number) => y > staleThreshold)) {
      issues.push({
        severity: 'warning',
        check: 'stale-figure',
  ...
  if (frontmatter.lastEdited) {
    const lastEdit = new Date(frontmatter.lastEdited as string);
  ...
  if (hasNetWorthRow && (filePath.includes('/people/') || filePath.includes('/funders/'))) {
    for (const asset of MAJOR_ASSETS) {
      if (asset.pattern.test(body)) {
        ...
        const assetLines = body.split('\n').filter((l: string) => asset.pattern.test(l));
        const hasValuation = assetLines.some((l: string) => /\\\$[\d.,]+\s*[BMTbmt]/i.test(l) || /billion|million/i.test(l));
...
export function runCheck(options: ValidatorOptions = {}): ValidatorResult {
  ...
  const financialFiles = files.filter((f: string) => isFinancialPage(f) && !f.endsWith('index.mdx'));
  ...
  if (ciMode) {
    ...
    if (allIssues.length === 0) {
      ...
      // Warnings first
      const warnings = allIssues.filter((a: FileIssues) => a.issues.some((i: FinancialIssue) => i.severity === 'warning'));
      const infos = allIssues.filter((a: FileIssues) => !a.issues.some((i: FinancialIssue) => i.severity === 'warning'));
      ...
      if (warnings.length > 0) {
        ...
        for (const { file, issues } of warnings) {
          ...
          for (const issue of issues.filter((i: FinancialIssue) => i.severity === 'warning')) {
      ...
      if (infos.length > 0) {
        ...
        for (const { file, issues } of infos) {
          ...
          for (const issue of issues.filter((i: FinancialIssue) => i.severity === 'info')) {
