#!/usr/bin/env node
/**
 * Gate rule: check sourcing coverage of TableBase submissions.
 *
 * Reads JSON manifests from data/tablebase-manifests/ and reports:
 * - Per-manifest: table, record count, sourcing coverage
 ...
 *
 * Exit codes:
 *   0 = All records have sourcing (or no manifests, or --force overrides soft block)
...
function main() {
  ...
  for (const file of files) {
    ...
    totalVerified += sourcingSummary.withSourcing;
    totalUnverified += sourcingSummary.withoutSourcing;

    if (sourcingSummary.withoutSourcing > 0) {
      warnings.push(
        `${file}: ${sourcingSummary.withoutSourcing}/${recordCount} ${table} records submitted WITHOUT sourcing`
      );
  ...
  if (warnings.length === 0) {
    console.log('All TableBase submissions have sourcing.');
    process.exit(0);
  }
  ...
  if (hardBlockedTables.size > 0) {
    ...
    console.log('Run `pnpm crux tb verify <table>` to sourcing records before submitting.');
    process.exit(1);
  }
  ...
  if (enforcement === 'soft' || enforcement === 'hard') {
    if (forceOverride) {
      console.log('');
      console.log('--force: sourcing warnings overridden.');
...
function logForceUsage(warnings: string[]): void {
  try {
    ...
    let logFile: string;
    try {
      logFile = execSync('git rev-parse --git-path sourcing-force-log', {
        cwd: PROJECT_ROOT,
        encoding: 'utf-8',
      }).trim();
    } catch {
      // Fallback if git is unavailable
      logFile = join(PROJECT_ROOT, '.git', 'sourcing-force-log');
    }
    ...
    console.warn(`[sourcing-coverage] Could not write force-log: ${e instanceof Error ? e.message : String(e)}`);
  }
}
