/**
 * Tests for validate-sourcing-coverage.ts enforcement logic.
 *
...
function runValidator(...args: string[]): { exitCode: number; output: string } {
  try {
    const output = execSync(
      `npx tsx crux/validate/validate-sourcing-coverage.ts ${args.join(' ')}`,
...
describe('validate-sourcing-coverage', () => {
  beforeEach(() => {
    backupManifests();
    cleanupManifests();
  });
  ...
  it('exits 0 when all records have sourcing', () => {
  ...
  it('exits 1 in soft enforcement when records lack sourcing (non-hard table)', () => {
    ...
    expect(result.output).toContain('3/5 investments records submitted WITHOUT sourcing');
  ...
  it('exits 0 in soft enforcement with --force override (non-hard table)', () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.output).toContain('--force: sourcing warnings overridden');
  });
  ...
  it('advisory mode exits 0 even with unverified records (non-hard table)', () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.output).toContain('5/5 investments records submitted WITHOUT sourcing');
  });
