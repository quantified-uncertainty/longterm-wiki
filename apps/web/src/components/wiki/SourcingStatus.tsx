...
/**
 * SourcingStatus — compact summary of entity-level sourcing verdicts.
 *
 * Client component that fetches sourcing verdicts for an entity and renders
 * a summary section on wiki entity pages. Shows nothing if no verdicts exist.
 */
export function SourcingStatus({ entityId }: SourcingStatusProps) {
  ...
  return (
    <section className="not-prose mt-6 mb-4" aria-labelledby="sourcing-status-heading">
      <div className="rounded-lg border border-border/60 bg-card/50">
        {/* Header */}
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/40">
          <ShieldCheck size={14} className="text-emerald-500/70" />
          <h2
            id="sourcing-status-heading"
        ...
        <div className="px-4 py-2.5 flex items-center gap-3 flex-wrap">
          {sortedVerdicts.map(([verdict, count]) => {
            const style = getSourcingVerdictStyle(verdict);
            return (
              <span
                key={verdict}
                className={cn(
          ...
          {showDetails && (
            <div className="px-4 pb-3 overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {verdicts.map((v, i) => {
                    const style = getSourcingVerdictStyle(v.verdict);
                    return (
                      <tr
