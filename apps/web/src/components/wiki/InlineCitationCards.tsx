...
import type { FootnoteEntry, FootnoteSourceEntry } from "@data";
...
/** Render a hover card for a footnote ref, merging verification + source metadata */
function FootnoteCard({
  ...
  quote: CitationQuote;
  anchor: HTMLElement;
  footnoteData?: FootnoteEntry;
  sourceData?: FootnoteSourceEntry;
}) {
...
interface InlineCitationCardsProps {
  ...
  /** Footnote data from build-time index for enriching hover cards */
  footnotes?: Record<number, FootnoteEntry>;
  /** Source groups from build-time index */
 ...
 *
 * Enhanced version of CitationOverlay that merges resource metadata from
 * the build-time footnoteIndex with verification data from the wiki-server.
 *
...
export function InlineCitationCards({
  ...
  // Build source lookup: footnoteNum → source group
  const fnToSource = new Map<number, FootnoteSourceEntry>();
  if (sources) {
    for (const src of sources) {
      for (const num of src.footnoteNumbers) {
        fnToSource.set(num, src);
      }
    }
  }
  ...
  return (
    <>
      {refAnchors.map(({ wrapper, quote, footnoteNum }) => (
        <FootnoteCard
          key={`fn-${quote.footnote}`}
          quote={quote}
          anchor={wrapper}
          footnoteData={footnotes?.[footnoteNum]}
          sourceData={fnToSource.get(footnoteNum)}
        />
      ))}
    </>
  );
