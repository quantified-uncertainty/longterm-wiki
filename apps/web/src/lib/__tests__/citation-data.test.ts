...
describe("getCitationQuotes", () => {
  ...
  it("returns all quotes including those without sourcing data", async () => {
    vi.resetModules();
    const localQuotes = [
      makeQuote({ footnote: 1, quoteVerified: true }), // has sourcing
      makeQuote({ footnote: 2 }), // no sourcing data — still included (NULL-verdict dots)
