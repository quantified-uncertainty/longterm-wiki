...
import { FactValueDisplay } from "@/components/directory";
...
export default async function ProjectDetailPage({
  ...
  const metadata = (
    <>
      ...
      {foundedDateText && <span>Founded {foundedDateText}</span>}
    </>
  );

  const headerLinks = [
    ...(url
      ? [{ label: safeHostname(url), href: url, external: true }]
      : []),
    ...(wikiHref ? [{ label: "Wiki article", href: wikiHref }] : []),
    { label: "Data", href: `/projects/${slug}/data` },
  ];
