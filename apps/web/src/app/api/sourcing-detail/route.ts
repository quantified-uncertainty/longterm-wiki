...
/**
 * GET /api/sourcing-detail?recordType=...&recordId=...
 *
 * Proxies sourcing evidence requests to the wiki-server's
 * /api/sourcing/evidence/:recordType/:recordId endpoint.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const url = `${config.serverUrl}/api/sourcing/evidence/${encodeURIComponent(recordType)}/${encodeURIComponent(recordId)}`;
