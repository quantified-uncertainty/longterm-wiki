...
// ── Verdict styling ────────────────────────────────────────────────────────
// Extends the shared sourcing styles with lucide icons for the action queue.
    ...
    void (async () => {
      try {
        ...
        const [contradictedRes, outdatedRes, recheckRes] = await Promise.all([
          fetch("/api/sourcing-verdicts-proxy?verdict=contradicted&limit=20"),
          fetch("/api/sourcing-verdicts-proxy?verdict=outdated&limit=10"),
          fetch("/api/sourcing-verdicts-proxy?verdict=partial&limit=10"),
        ]);
...
function ActionItem({ item, names }: { item: VerdictRow; names: NameMap }) {
  ...
  const href = `/sourcing/${encodeURIComponent(item.recordType)}/${encodeURIComponent(item.recordId)}`;
