...
import type { RecordRow } from "./factbase-records-content";
...
const columns: ColumnDef<RecordRow>[] = [
  {
    ...
    cell: ({ row }) => (
      <Link
        href={`/factbase/record/${row.original.recordKey}`}
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Link
        href={`/factbase/entity/${row.original.entityId}`}
...
export function FBRecordsTable({ data }: { data: RecordRow[] }) {
