...
--
-- Idempotent: uses ON CONFLICT DO NOTHING / WHERE NOT EXISTS throughout.
...
-- 1. Enable pgcrypto for SHA-256 hash computation (outside transaction — DDL)
...
-- Uses WHERE NOT EXISTS to avoid conflicts on BOTH the id PK and the url unique index.
-- (ON CONFLICT can only target one constraint; a URL that already exists in resources
-- with a different id would cause an unhandled unique violation.)
...
WHERE NOT EXISTS (
  ...
  WHERE r.url = COALESCE(ds.fetch_url, 'urn:lw:tabular-source:' || ds.id)
)
ON CONFLICT (id) DO NOTHING;

-- 3. Create resource_tabular_sources rows.
-- Looks up the resource_id via URL match (handles both fresh inserts and pre-existing resources).
...
SELECT
  ...
  ds.updated_at
FROM data_sources ds
JOIN resources r ON r.url = COALESCE(ds.fetch_url, 'urn:lw:tabular-source:' || ds.id)
...
-- Verify (outside transaction so it runs even if we're checking post-commit state)
DO $$
DECLARE
  ds_count integer;
  rts_count integer;
