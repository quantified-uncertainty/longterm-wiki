 ...
 * Uses ON CONFLICT (id) DO UPDATE — the canonical `id` is generated by
 * each domain handler and is the strongest natural key. The
 * `tablebase/things.ts /sync` route separately uses ON CONFLICT
 * (sourceTable, sourceId) because its callers (the crux reconciliation
 * CLI) only know the source pointer pair, not the generated id.
 */
