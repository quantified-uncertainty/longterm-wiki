/**
 * Shared test fixtures for facts endpoints (REST and oRPC).
 *
 * Contains the in-memory facts store and SQL dispatch function used by
 * both facts.test.ts and orpc-facts.test.ts, eliminating duplication.
 */
...
export function resetFactsStore() {
  factsStore = new Map();
  nextId = 1;
}
...
// ---------------------------------------------------------------------------
// SQL dispatch — simulates Drizzle/Postgres query execution
// ---------------------------------------------------------------------------

export function factsDispatch(query: string, params: unknown[]): unknown[] {
  ...
  if (
    ...
    results.sort((a, b) => {
      const cmp = String(a.entity_id || "").localeCompare(
        String(b.entity_id || "")
      );
      if (cmp !== 0) return cmp;
...
/** Seed a fact via the REST sync endpoint. */
export function seedFact(
  app: Hono,
  entityId: string,
  factId: string,
