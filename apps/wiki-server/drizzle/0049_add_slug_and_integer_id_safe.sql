-- Phase 4a: Expand schema for wiki_pages integer PK migration (safe/idempotent)
-- Epic: #1497 (Convert wiki_pages.id from text slug to integer PK)
-- Issue: #1498
--
-- Idempotent version of 0048 — all DDL uses IF NOT EXISTS / IF EXISTS so this
-- migration can be retried safely after partial application or lock timeouts.
--
-- This migration:
...
-- (WHERE ... IS NULL makes these idempotent)
-- ============================================================

UPDATE citation_quotes cq
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = cq.page_id AND cq.page_id_int IS NULL;
--> statement-breakpoint

UPDATE citation_accuracy_snapshots cas
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = cas.page_id AND cas.page_id_int IS NULL;
--> statement-breakpoint

UPDATE edit_logs el
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = el.page_id AND el.page_id_int IS NULL;
--> statement-breakpoint

UPDATE hallucination_risk_snapshots hrs
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = hrs.page_id AND hrs.page_id_int IS NULL;
--> statement-breakpoint

UPDATE session_pages sp
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = sp.page_id AND sp.page_id_int IS NULL;
--> statement-breakpoint

UPDATE auto_update_results aur
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = aur.page_id AND aur.page_id_int IS NULL;
--> statement-breakpoint

UPDATE claim_page_references cpr
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = cpr.page_id AND cpr.page_id_int IS NULL;
--> statement-breakpoint

UPDATE resource_citations rc
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = rc.page_id AND rc.page_id_int IS NULL;
--> statement-breakpoint

UPDATE auto_update_news_items auni
SET routed_to_page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = auni.routed_to_page_id AND auni.routed_to_page_id_int IS NULL;
--> statement-breakpoint

UPDATE page_improve_runs pir
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = pir.page_id AND pir.page_id_int IS NULL;
--> statement-breakpoint

UPDATE page_citations pc
SET page_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = pc.page_id AND pc.page_id_int IS NULL;
--> statement-breakpoint

UPDATE page_links pl
SET source_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = pl.source_id AND pl.source_id_int IS NULL;
--> statement-breakpoint

UPDATE page_links pl
SET target_id_int = ei.numeric_id
FROM entity_ids ei
WHERE ei.slug = pl.target_id AND pl.target_id_int IS NULL;
--> statement-breakpoint
