...
-- Each row records one LLM check of a fact against a specific resource.
-- A fact can have multiple verification rows (one per resource checked).

CREATE TABLE IF NOT EXISTS kb_fact_resource_verifications (
...
CREATE INDEX IF NOT EXISTS idx_kbfrv_fact_id ON kb_fact_resource_verifications(fact_id);
CREATE INDEX IF NOT EXISTS idx_kbfrv_verdict ON kb_fact_resource_verifications(verdict);

-- Aggregate per-fact verdict, derived from per-resource verifications.
-- One row per fact. Recomputed periodically from kb_fact_resource_verifications.
