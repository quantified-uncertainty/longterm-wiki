...
import { PROJECT_ROOT } from '../content-types.mjs';
 ...
 */
function loadCanonicalFacts() {
  const facts = [];

  if (!existsSync(FACTS_DIR)) return facts;

  const files = readdirSync(FACTS_DIR).filter(f => f.endsWith('.yaml'));
 ...
 */
function generateSearchPatterns(value) {
  ...
  if (numUnitMatch && !dollarMatch) {
    ...
    const abbrevMap = { billion: 'B', million: 'M', trillion: 'T', thousand: 'K' };
    const abbr = abbrevMap[unit];
    if (abbr) {
      patterns.push({
 ...
 */
function isInsideFComponent(body, matchIndex) {
 ...
 */
function isInCodeOrComponent(body, matchIndex) {
