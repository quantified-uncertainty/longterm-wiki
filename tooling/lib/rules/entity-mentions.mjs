 ...
 */
function buildEntityLookup(engine, contentFiles) {
 ...
 */
function isInEntityLink(content, position) {
 ...
 */
function isInJsxTag(content, position) {
  const before = content.slice(0, position);
 ...
 */
function isInMarkdownLink(content, position) {
 ...
 */
function findMentions(content, entityTitle, entityId) {
  const mentions = [];
...
export const entityMentionsRule = createRule({
  ...
  check(contentFiles, engine) {
    ...
    for (const content of contentFiles) {
      ...
      const fileMentions = new Map(); // entityId -> first mention
