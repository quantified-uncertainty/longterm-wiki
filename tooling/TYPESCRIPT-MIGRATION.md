...
The `tooling/` directory contains the crux CLI (~98 files, ~625KB) for validation, analysis, content generation, and resource management. It was ported from the old cairn/Astro monorepo to the current Next.js setup across PRs #3-#8.
...
2. Run TS files with `tsx` (already available):
   ...
   - Or rename `crux.mjs` → `crux.ts` and run via `tsx tooling/crux.ts`

3. Add a typecheck script: `"typecheck:tooling": "tsc --noEmit -p tooling/tsconfig.json"`
