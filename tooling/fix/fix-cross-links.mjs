 ...
 *
 * Usage:
 *   node tooling/crux.mjs fix cross-links              # Preview changes
 *   node tooling/crux.mjs fix cross-links --apply      # Apply changes
 *   node tooling/crux.mjs fix cross-links --verbose    # Show detailed matches
 *   node tooling/crux.mjs fix cross-links --file path  # Fix single file
 *   node tooling/crux.mjs fix cross-links --fuzzy      # Include fuzzy suggestions
 *
 * Rules:
...
async function main() {
  ...
  if (APPLY_MODE && totalChanges > 0) {
    ...
    console.log(`${colors.dim}Run 'node tooling/crux.mjs validate compile --quick' to verify${colors.reset}`);
  }
}
