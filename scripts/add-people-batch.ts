 ...
 *
 * Generates:
 * - packages/factbase/data/fb-entities/{slug}.yaml for each person
...
function lookupOrgStableId(slug: string): string | undefined {
  ...
  const path = resolve(ROOT, `packages/factbase/data/fb-entities/${slug}.yaml`);
...
async function main() {
  const kbThingsDir = resolve(ROOT, "packages/factbase/data/fb-entities");
