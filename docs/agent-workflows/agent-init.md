...
4. **Signals start on the Linear issue** (primary) — auto-detects a Linear ID from the branch name (`claude/qua-NNN-*` or `codex/qua-NNN-*`) or task description (any bare `QUA-NNN` token), or takes `--linear=QUA-NNN` explicitly. Moves the issue to "In Progress" and posts a start comment. Best-effort: a Linear failure does not fail init.
...
- **Working on a Linear issue** (most common): start a branch `claude/qua-NNN-description` or `codex/qua-NNN-description` and the ID is detected automatically, or pass `--linear=QUA-NNN` explicitly
