...
**Phase 4 deletion-candidate triage:** of the 23 S and 10 W validators, **9 are recommended for follow-up tickets in Phase 4**. Two selection criteria are accepted (a candidate must meet at least one):

1. **Advisory-and-stale**: gate-advisory AND no bug-fix touch in ≥30 days. (Used for `validate-cross-page-dates`, `validate-numeric-consistency`, `validate-stale-content`, `validate-resource-quality`, `validate-entity-refs`, `validate-factbase-entities`, `validate-mdx-compile`.)
2. **Gate-blocking-but-clear-runtime-target**: the validator's docstring identifies a specific off-gate home (an ingester pipeline, a sync handler, a loader). (Used for `validate-framework-thresholds` + `validate-framework-versions` — both gate-blocking with bug fixes ≤1 week old, but both have explicit "belongs in the QUA-711 ingester" pointers in their own docstrings.)

The other 24 S/W validators have no migration target ready yet (the schema/loader/handler doesn't exist or hasn't been designed) — they stay in the gate until their migration target is built. The 9 candidates are listed in the "Phase 4 follow-ups" section at the bottom.

## Disposition table

Columns: name | current-runs-in | has-test | last-touch | last bug-fix | S/W/R | recommendation | rationale.

`current-runs-in`: `gate-blocking` | `gate-advisory` | `not-in-gate (orchestrator)`. Three validators are orchestrators/runners rather than checks: `validate-daily` (runs the daily local+server suite, only via `pnpm crux validate daily`), `validate-data` (runs entity data integrity, only via `pnpm crux validate data`), and `validate-unified` (dispatches the `crux/lib/rules/` ruleset; called from gate as `pnpm crux validate unified --rules=…` via the `unified-blocking` step but not by direct file path). All three are tagged `not-in-gate (orchestrator)` to flag that they're not point-checks the gate runs in isolation.
...
| validate-daily | not-in-gate (orchestrator) | no | 2026-05-02 | 2026-05-02 | **R** | keep | Orchestrator, not a check. Runs the daily local+server suite. |
...
| validate-data | not-in-gate (orchestrator) | yes | 2026-04-16 | 2026-04-15 | **R** | keep (or fold into daily) | Standalone runner exposed via `crux validate data`. Reached only via daily; could be inlined there in Phase 4 with no behaviour change. |
| validate-directory-pages | gate-advisory | yes | 2026-04-30 | 2026-04-30 | **W** | migrate to /internal dashboard | Sparse-directory + missing-fields advisory. Better as a `/internal/data-quality` row than as a gate step. |
...
| validate-factbase-record-refs | gate-blocking | yes | 2026-04-29 | 2026-04-29 | **S** | migrate to sync-handler refusal | sid_ refs that don't resolve = sync-handler 4xx territory. Recently bug-fixed; keep gate version until sync handlers refuse. |
...
| validate-no-sourcinged | gate-blocking | yes | 2026-04-30 | 2026-04-30 | **R** | keep (regression lock) | Small QUA-237 mass-rename regression check (typos that QUA-897 corrected). Not a carveout-named ratchet — informal usage. Cheap to keep. |
...
| validate-person-refs | gate-blocking | yes | 2026-04-16 | 2026-04-16 | **S** | migrate to FactBase Zod | Person `!ref` resolution + display-name presence. FactBase loader / sync-handler concern. |
...
| validate-prompt-escaping | gate-blocking | yes | 2026-03-29 | 2026-03-29 | **R** | keep | Banned-pattern security check (XML interpolation). Static-only. |
...
| validate-rendered-sid | gate-blocking | yes | 2026-04-04 | 2026-04-04 | **S** | migrate to build-data fail-closed | "Last-line-of-defence" check on built JSON. Move into `apps/web/scripts/build-data.mjs` fail-closed checks (already partially fail-closed per QUA-772). |
| validate-resource-quality | gate-advisory | yes | 2026-05-03 | 2026-03-21 | **W** | migrate to scraper pipeline | HTML in titles, scraping artifacts. Belongs in the resource-ingestion path, not the gate. The errors are content-level and the warnings are informational, so the migration target is "fail at scrape time, not at gate time" rather than a hard delete. **Phase 4 candidate.** |
...
| validate-soft-fks | gate-advisory | yes | 2026-04-29 | 2026-03-29 | **S** | migrate to hard FKs | The validator name itself names the migration target. Likely a multi-PR migration; not on the Phase 4 short-list because the migration itself is bigger than a deletion candidate's scope. |
...
| validate-table-formatting | gate-blocking | yes | 2026-05-01 | 2026-05-01 | **R** | keep (regression lock) | Locks down the QUA-1006 sweep. Recently-landed regression-prevention. Not a carveout-named ratchet — informal usage. |
| validate-table-states | gate-blocking | yes | 2026-05-02 | 2026-05-02 | **R** | keep (regression lock) | Locks down the QUA-1008 sweep. Same shape as table-formatting. Not a carveout-named ratchet — informal usage. |
...
| validate-things-denorm-dead | gate-blocking | yes | 2026-04-24 | — | **R** | keep (regression lock) | Banned re-introduction of dropped columns (QUA-507). Static-only. Not a carveout-named ratchet — informal usage. |
...
| validate-url-normalize | gate-blocking | yes | 2026-04-16 | 2026-04-16 | **R** | keep (regression lock) | Locks down the QUA-341 helper consolidation. Banned-pattern static. Not a carveout-named ratchet — informal usage. |
| validate-verdict-priority | gate-blocking | yes | 2026-04-28 | 2026-04-28 | **R** | keep (regression lock) | Banned distinct verdict-priority maps (QUA-429). Static. Not a carveout-named ratchet — informal usage. |
...
The following 9 validators are filed as **QUA-1093** ("ADR-0003 Phase 4: 9 W/S-class deletion candidates from Phase 3 disposition") — a single batch ticket per **QUA-1092** § "Deliverable" ("For each W-class deletion candidate identified, file one Linear sub-ticket (or batch into a single 'Phase 3 W-class deletions' ticket if the count is small)"). The batching authorization comes from the dispatch ticket, not from ADR-0003 itself (the ADR's Phase 3 wording is the stricter "File one ticket per W-class deletion candidate" with no batching exception). The count is small enough — 7 W-class + 2 S-class with clear migration targets — to warrant a single batch ticket.
...
- **No-test signal.** 20 of 67 (30%) have no `.test.ts` sibling, counting both adjacent-sidecar and `crux/validate/__tests__/<name>.test.ts` placement. This is *not* the same denominator the codebase scout used: the scout reported 48/73 (66%) coverage over the full 73 `validate-*.ts` set including the 5 orphans + the gate orchestrator; in this 67-validator set, coverage is 47/67 (70%). Of the 20 no-test validators, 5 are recommended Phase 4 candidates (so testing them is moot if they're being migrated out); the rest are `keep` recommendations and should ideally get tests in Phase 4. The largest no-test validator is `validate-component-refs.ts` (627 LOC) — already flagged in the codebase scout as a top-5 risk.
