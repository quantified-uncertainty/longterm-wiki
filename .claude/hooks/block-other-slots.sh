...
if echo "$COMMAND" | grep -qE '(cd|pushd)\s+.*/lw/a[0-9]+' ; then
  ...
  if [ "$TARGET" != "$CURRENT_SLOT" ] && [ -n "$TARGET" ]; then
    echo "BLOCKED: Cannot cd into slot $TARGET — it may have an active agent session." >&2
    echo "  You own slot $CURRENT_SLOT only. Other slots are off-limits." >&2
    echo "  For branch isolation, use /tmp/ worktrees from lw/main instead." >&2
    exit 2
  fi
fi
...
if echo "$COMMAND" | grep -qE '(pkill|killall)\s+.*(codex|claude|next|node)'; then
  echo "BLOCKED: pkill/killall commands targeting codex/claude/next/node are prohibited." >&2
  echo "  They can kill processes in other agent slots." >&2
  echo "  Kill specific PIDs instead, after verifying they belong to your session." >&2
  exit 2
fi
