...
- The same workflow pattern is used by `.github/workflows/refresh-frameworks.yml` (QUA-711 Phase 4-D): cron commented out, `workflow_dispatch`-only, **not** in the staleness list. Re-enabling requires the same checklist (add to `workflowFiles`, accept alert noise on missed weekly runs).
