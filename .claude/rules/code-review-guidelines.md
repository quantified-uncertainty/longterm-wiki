...
- **Address review feedback before merging, not in follow-up PRs.** If CodeRabbit or a human reviewer flags an issue, fix it in the same PR. A follow-up PR for a 2-line CodeRabbit fix means the original PR was merged prematurely. The only exception: review comments that require a design decision or are out of scope for the current PR — leave a reply explaining why and file an issue if needed.
