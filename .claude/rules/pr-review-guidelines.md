...
## Phase 1: Self-Review (run `/paranoid-pr-review`)

Before the final push, run the `/paranoid-pr-review` slash command. This triggers the paranoid 6-step code review defined in `.claude/commands/paranoid-pr-review.md`.
...
If the review finds CRITICAL or WARNING issues: fix them, re-run `/paranoid-pr-review`, and repeat until the verdict is **GOOD TO GO**.

## Phase 2: Push & Monitor CI (run `/push-and-ensure-green`)

Once the review passes, run the `/push-and-ensure-green` slash command. This handles:
...
Do NOT consider work complete until `/push-and-ensure-green` confirms all CI checks are green.
...
1. `/paranoid-pr-review` — fix all issues found
