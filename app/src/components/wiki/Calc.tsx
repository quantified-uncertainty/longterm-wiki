...
interface CalcProps {
  ...
  expr: string;
  /** Format mode: "currency" ($X billion), "percent" (X%), "number" (X,XXX) */
  format?: CalcFormat;
 ...
 *   <Calc expr="{anthropic.revenue-arr-2025} * 2.5" format="currency" />
 *   <Calc expr="{anthropic.gross-margin}" format="percent" />
 */
export function Calc({ expr, format, precision, prefix, suffix, children, className }: CalcProps) {
  try {
    const result = calc(expr, getFact, { format, precision, prefix, suffix });
    const displayValue = children || result.display;

    // Build human-readable formula for tooltip
    const formulaDisplay = result.inputs.reduce(
      (str, input) => str.replace(`{${input.ref}}`, input.value || String(input.numeric)),
      expr
    );
            ...
            <span className="block border-t border-border pt-1.5 mt-1">
              {result.inputs.map((input) => (
                <span key={input.ref} className="block text-muted-foreground mt-0.5">
                  ...
                  {input.asOf && (
                    <span className="text-muted-foreground/60 ml-1">({input.asOf})</span>
                  )}
                </span>
              ))}
            </span>
