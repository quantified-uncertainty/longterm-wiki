...
// ---------------------------------------------------------------------------
// Tokenizer + recursive-descent evaluator (no eval)
// ---------------------------------------------------------------------------
  ...
  if (format === "currency") {
    ...
    if (precision != null) {
      if (abs >= 1e12) formatted = `${sign}\$${(abs / 1e12).toFixed(precision)} trillion`;
        ...
        formatted = `${sign}\$${abs.toLocaleString("en-US", {
    ...
    // Auto-detect
    formatted = precision != null ? n.toFixed(precision) : autoFormat(n);
  }
