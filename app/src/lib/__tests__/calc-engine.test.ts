...
describe("calc — expression evaluation", () => {
  it("evaluates a simple fact reference", () => {
    ...
    expect(result.value).toBe(350_000_000_000);
  });

  it("evaluates division of two facts", () => {
    const result = calc("{anthropic.valuation} / {anthropic.revenue-arr-2025}", mockLookup);
    expect(result.value).toBeCloseTo(38.89, 1);
  });
  ...
  it("evaluates addition and subtraction", () => {
    const result = calc("{anthropic.valuation} - {anthropic.revenue-arr-2025}", mockLookup);
    expect(result.value).toBe(341_000_000_000);
  });

  it("evaluates parenthesized expressions", () => {
    const result = calc("({anthropic.valuation} + {anthropic.revenue-arr-2025}) / 2", mockLookup);
    expect(result.value).toBe(179_500_000_000);
  });
...
describe("calc — error handling", () => {
  ...
  it("throws on division by zero", () => {
    expect(() => calc("{anthropic.valuation} / 0", mockLookup)).toThrow("Division by zero");
  });

  it("throws on unexpected characters", () => {
    expect(() => calc("{anthropic.valuation} @ 2", mockLookup)).toThrow("Unexpected character");
  });
});
...
describe("calc — input tracking", () => {
  it("tracks all referenced facts", () => {
    ...
    expect(result.inputs[0].value).toBe("$350 billion");
    expect(result.inputs[0].asOf).toBe("2026-01");
...
describe("formatValue", () => {
  it("auto-formats large numbers with scale suffix", () => {
    expect(formatValue(350_000_000_000)).toBe("350.0 billion");
  ...
  it("formats currency with $ prefix", () => {
    expect(formatValue(350_000_000_000, { format: "currency" })).toBe("$350.0 billion");
    expect(formatValue(9_000_000_000, { format: "currency" })).toBe("$9.0 billion");
  ...
  it("formats percentages from decimal", () => {
    ...
    expect(formatValue(0.883, { format: "percent", precision: 1 })).toBe("88.3%");
  });
  ...
  it("formats negative currency", () => {
    expect(formatValue(-5_000_000_000, { format: "currency" })).toBe("-$5.0 billion");
  });
});
...
describe("calc — formatted output", () => {
  it("formats P/S ratio with suffix", () => {
    ...
    expect(result.display).toBe("39x");
  });

  it("formats gross margin as percent", () => {
    const result = calc("{anthropic.gross-margin}", mockLookup, { format: "percent" });
    expect(result.display).toBe("40%");
  });

  it("formats revenue as currency", () => {
    const result = calc("{anthropic.revenue-arr-2025}", mockLookup, { format: "currency" });
